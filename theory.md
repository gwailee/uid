...
#### C6.4-bis.1 世界模型：CID 在"旋度关闭、欧姆记忆、单尺度噪声"极限下的隐空间预测流

**第一步：写出世界模型的动力学骨架。** 世界模型（[Ha & Schmidhuber，2018](https://arxiv.org/abs/1803.10122)；[Hafner 等，2023，DreamerV3](https://arxiv.org/abs/2301.04104)）的核心是一个在隐空间 $z$ 上学习的状态转移分布 $p(z_{t+1}\mid z_t, a_t)$，并以重构/预测损失训练。其连续时间形式是一个由当前隐状态驱动、带高斯过程噪声的递归预测流。在 CID 中，这对应：令旋度 $v\equiv 0$、取指数（欧姆）记忆核（与 Mamba 同款，式 C6.11）、噪声取单尺度高斯，势能取由重构损失定义的二次型 $U(z)=\tfrac12 (z-\mu_\theta)^\top M (z-\mu_\theta)$。其隐空间演化即下式，标记 (C6.20a)：

$$
\frac{dz}{dt} = -\nabla U(z) - \int_0^t \gamma_0 e^{-(t-s)/\tau_c}\,\dot z(s)\,ds + \xi_{\mathrm{white}}(t), \qquad v \equiv 0
$$

**第二步：识别其与 Mamba/扩散模型的关系。** 比较 (C6.20a) 与 Mamba 的 (C6.14)、扩散模型的 (C6.15) 可见：世界模型在结构上恰是"Mamba 的指数记忆 + 扩散模型的单尺度噪声"的并联——它同时保留了**短记忆**与**白噪声**两项，却仍**关闭旋度**。因此其"预测"本质上是沿保守势能的去噪式回归，预测信息 $I_{\mathrm{pred}}$ 由命题 C3.3 仍被细致平衡锁死为零。

**结论**：世界模型是 CID 主方程在"旋度关闭、指数（欧姆）记忆、单尺度白噪声、二次（重构）势能"极限下的特解。它比 Transformer 多保留了记忆与噪声两项，是三大基础架构中**最接近完整 CID 的一个**，但仍缺最关键的旋度。这从统一框架解释了一个经验现象：世界模型在**短程滚动预测**上表现良好，却在**长程自洽想象**（需要内禀非保守环流维持的、不坍缩到势阱的持续探索）上系统性退化——因为支撑长程自洽的正是它砍掉的旋度项。世界模型的位置，下式标记 (C6.20b)：

$$
\text{世界模型} = \text{CID} \mid (v = 0,\ \text{指数记忆},\ \text{单尺度白噪声})
$$

#### C6.4-bis.2 JEPA：CID 在"旋度关闭、δ 记忆、无噪声、隐空间能量势"极限下的预测梯度流

**第一步：写出 JEPA 的能量骨架。** 联合嵌入预测架构（JEPA，[LeCun，2022](https://openreview.net/forum?id=BZ5a1r-kVsf)；[Assran 等，2023，I-JEPA](https://arxiv.org/abs/2301.08243)）在隐空间预测目标表征，并以一个能量函数 $U_{\mathrm{JEPA}}(z)=\tfrac12\lVert z - \mathrm{Pred}(z_{\mathrm{ctx}})\rVert^2$ 度量预测嵌入与目标嵌入的失配。其训练即对该能量作梯度下降。在 CID 中，这对应令 $v\equiv 0$、$\gamma\to\delta$ 核、$\xi\to 0$，势能取 JEPA 失配能量，得纯梯度流（与 Transformer 同款，式 C6.5），下式标记 (C6.20c)：

$$
\frac{dz}{dt} = -\nabla U_{\mathrm{JEPA}}(z), \qquad v \equiv 0,\ \xi \equiv 0
$$

**第二步：与 Transformer 的同构。** (C6.20c) 与 Transformer 的 (C6.5) 在动力学骨架上**完全同构**，区别仅在势能 $U$ 的具体形式（Hopfield 能量 vs. 隐空间失配能量）。这印证 C6.6 的"唯一决定"口径：势能形状属第二层物理输入，可自由选取，但**不改变骨架所处的极限坐标**。

**结论**：JEPA 是 CID 主方程在"旋度关闭、δ 记忆、无噪声、隐空间预测能量"极限下的特解，与 Transformer 同属"全部三项关闭"族。须明确：JEPA 把目标从"重构像素"改为"预测隐空间表征"，这在工程上是重要进步（避免了像素级噪声），但**在 CID 极限坐标上未移动一步**——它仍是 $v\equiv 0$ 的保守梯度流，故其"预测"在命题 C3.3 意义下仍非内禀预测信息。JEPA 的位置，下式标记 (C6.20d)：

$$
\text{JEPA} = \text{CID} \mid (v = 0,\ \delta\ \text{记忆},\ \text{无噪声})
$$

#### C6.4-bis.3 推理增强模型：CID 在"单步特解上的外循环迭代"——仍未触及旋度

**第一步：区分单步与外循环两个层次。** 推理增强模型（DeepSeek-R1、o1–o3）通过思维链（CoT）/强化学习在**推理时**展开多步采样与回溯。须严格区分两个层次：**每一步**的前向计算仍是标准 Transformer，即 CID 的 (C6.10)（$v\equiv 0$）；多步推理是在该单步特解之外叠加的**离散外循环搜索**。设外循环为 $z^{(k+1)} = G(z^{(k)})$，其中单步映射 $G$ 即 (C6.10)。

**第二步：外循环不构成连续旋度。** 关键观察是：离散外循环的迭代 $G$ 是否等价于在**连续动力学层**装回旋度 $v(\varphi)$？答案为否。由 C 第 3 章，旋度的物理判据是稳态概率流的非零环量 $\oint J\cdot dl\neq 0$（细致平衡破缺）。而 CoT 外循环的每一步 $G$ 仍由保守势能梯度生成（$v\equiv 0$），其多步复合 $G^{\circ k}$ 仍是保守映射的复合，**不产生连续动力学层的稳态环流**。换言之，推理增强是用**外部串行算力**去近似本应由内禀非保守流免费提供的预测能力——这恰是 C14.2 所述"靠堆叠去近似动力学应免费提供之物"的另一种形态，只不过堆叠的不是参数而是推理步数。

**结论**：推理增强模型是 CID 单步特解 (C6.10) 之上的离散外循环，其单步动力学仍处"旋度关闭"极限，故未突破命题 C3.3 锁定的内禀预测上限；它以**推理时算力**换取表观推理能力，但这一交换发生在 softmax-attention 接口之内，仍受 Alman-Song-Gupta 二次复杂度墙约束。推理增强的位置，下式标记 (C6.20e)：

$$
\text{推理增强模型} = \big[\,\text{CID} \mid (v = 0,\ \delta\ \text{记忆},\ \text{无噪声})\,\big]^{\circ k}\ (\text{外循环 }k\text{ 步})
$$

#### C6.4-bis.4 稀疏路由（MoE）：CID 在"势能分块、旋度仍为零"极限下的特解

**第一步：识别 MoE 的物理含义。** 稀疏路由架构（MoE，[Shazeer 等，2017](https://arxiv.org/abs/1701.06538)）由门控网络在多个专家子网络间稀疏选择。在 CID 中，这对应把势能 $U(\varphi)$ 取为分块结构 $U(\varphi)=\sum_e g_e(\varphi)\,U_e(\varphi)$，其中 $g_e$ 为门控权重、$U_e$ 为各专家的局部势能。门控选择即在状态空间不同区域切换有效势阱。

**第二步：分块势能不引入旋度。** 无论门控如何切换，每个专家分支仍是 $-\nabla U_e$ 的保守梯度流，门控加权的总流 $-\sum_e g_e\nabla U_e$ 在 $g_e$ 不显含速度、不破坏势的全微分性质时**仍是保守场**（$\nabla\times(-\nabla U)=0$）。因此 MoE 扩大了**容量**（更多专家、更复杂势面），却未改变 $v\equiv 0$ 这一极限坐标。

**结论**：MoE 是 CID 主方程在"势能分块、旋度仍为零"极限下的特解。它是对**势能 $U$ 表达力**的扩展，而非对**旋度 $v$** 的装回——这从统一框架解释了为何 MoE 能高效扩大参数容量，却不改变模型在命题 C3.3 意义下的内禀预测上限。MoE 的位置，下式标记 (C6.20f)：

$$
\text{MoE} = \text{CID} \mid \big(v = 0,\ \delta\ \text{记忆},\ U=\textstyle\sum_e g_e U_e\big)
$$

#### C6.5 统一图景：七类架构、七处取舍、一个共同缺陷

综合 C6.2–C6.4 与 C6.4-bis，当前几乎全部主流深度学习架构在 CID 极限坐标下的位置一目了然。三大基础架构的定位重述如下。
...
C6.4-bis 进一步把世界模型 (C6.20b)、JEPA (C6.20d)、推理增强模型 (C6.20e)、稀疏路由 MoE (C6.20f) 纳入同一坐标。七类架构在三个开关（旋度 v、记忆核 γ、噪声 ξ）下的取舍各异，覆盖了从"全部三项关闭"到"记忆+噪声并存"的多种组合，又分别在势能表达力（MoE 的分块势能）、预测目标（JEPA 的隐空间能量）、推理时算力（R1/o-系列的离散外循环）上做了不同扩展，但其**共同缺陷**清晰可见：旋度项 v 一律被设为零。

须强调这一缺陷的**普遍性与无例外性**：无论沿哪一条工程路线扩展——把记忆装回（Mamba、世界模型）、把噪声装回（扩散模型、世界模型）、改换预测目标（JEPA）、增加推理时算力（R1/o-系列）、还是扩大容量（MoE）——只要不重新装回旋度，七类架构无一例外停留在 $v\equiv 0$ 的同一特解族内。由命题 C3.2，$v \equiv 0$ 意味着系统满足细致平衡；由命题 C3.3，细致平衡意味着零内禀预测信息。这就是"注意力并不够"的完整物理论证链，下式标记 (C6.21)：

$$
\text{softmax 注意力} \subset \text{纯保守流 } (v\equiv 0) \;\Rightarrow\; \text{细致平衡 (C3.2)} \;\Rightarrow\; \sigma = 0 \;\Rightarrow\; I_{\mathrm{pred}} = 0
$$

其中最后一步由命题 C3.3 经熵产生率下界 (C3.14) 给出，不依赖马尔可夫假设。须特别说明：链条 (C6.21) 的前提已由 C6.4-bis 从"softmax 注意力"放宽为"任何 $v\equiv 0$ 的纯保守流"，因而它不仅适用于 Transformer，也同样适用于其余六类架构——记忆、噪声、外循环、分块势能均不改变 $v\equiv 0$ 这一锁死条件。

链条 (C6.21) 表明：任何停留在纯保守梯度流（$v\equiv 0$）框架内的架构，无论参数如何增大、数据如何增多、记忆与噪声如何补全、推理步数如何叠加、专家如何扩充，其内禀预测能力的物理上限都被细致平衡锁死。突破这一上限的唯一出路，是重新装回 CID 被砍掉的三项——首先是旋度 v(φ)。这正是 UID 所论证的架构层物理重构方向，也与 [Alman-Song（2023）](https://arxiv.org/abs/2302.13214)、[Gupta 等（2025）](https://arxiv.org/abs/2502.16963)的复杂度下界互补：后者表明 softmax 接口内的优化无法突破二次复杂度墙，前者（本文）表明要突破须脱离该接口、装回非保守动力学。这一突破口的可工程化实现与其 5–10 倍参数效率的量级来源，见第 14 章（特别是 C14.2 与 C14.2-bis）。
...
### C 第 14.2-bis 节：5–10 倍区间的量级推导

> **定位声明（必读）**：本节给出 5–10 倍承诺的**量级来源**。须诚实指出：这是一个**在明确假设下的量级估计（order-of-magnitude estimate）**，不是从公理严格导出的定理。它的作用是说明"5–10 倍"并非随意取数，而是由三个相互独立的物理论证收敛到同一量级；其最终裁决仍交由 C14.4 的判决性实验（测试二）。下面三条论证分别从**自由度计数**、**复杂度类**、**信息容量**三个角度给出同一区间。

#### C14.2-bis.1 论证一（自由度计数）：旋度免费提供的反对称自由度

由 (C14.1)，线性旋度由反对称生成元 $\Omega_A=-\Omega_A^\top$ 给出。在 $d$ 维隐空间中：

- 一个**保守**梯度流的稳态由对称正定矩阵（势能 Hessian）刻画，其独立自由度为对称矩阵的维数 $d(d+1)/2$；
- 装回旋度后，系统额外获得**反对称**部分 $\Omega_A$ 的自由度 $d(d-1)/2$。

关键在于（由 C14.2 的零参数旋度机制）：$\Omega_A$ 的这 $d(d-1)/2$ 个自由度**不需新增可训练参数**，由已有的多热浴温差结构派生。于是，在相同可训练参数预算下，CID 的有效动力学自由度相对纯梯度流的提升比为下式，标记 (C14.8)：

$$
R_{\mathrm{dof}} = \frac{\tfrac{d(d+1)}{2} + \tfrac{d(d-1)}{2}}{\tfrac{d(d+1)}{2}} = \frac{d^2}{\tfrac{d(d+1)}{2}} = \frac{2d}{d+1} \xrightarrow{d\gg 1} 2
$$

(C14.8) 给出"单层、单一旋度通道"下约 **2 倍**的下界。但旋度的收益在**深度方向累积**：纯梯度流要在 $L$ 层中逐层用参数近似非保守流，而零参数旋度在每层免费提供，故深度网络的累积比是单层比的若干倍叠加，量级落在 $2$–$3$ 倍之上（见论证三的乘性叠加）。这给出区间下端。

#### C14.2-bis.2 论证二（复杂度类）：脱离 softmax 接口的复杂度降阶

由 [Alman-Song（2023）](https://arxiv.org/abs/2302.13214)、[Gupta 等（2025）](https://arxiv.org/abs/2502.16963)，softmax 注意力在序列长度 $n$ 上有 $\Omega(n^2)$ 的复杂度下界。装回旋度后，CID 的非保守环流由局部反对称耦合 (C14.3) 实现，其计算复杂度为 $O(n\cdot d)$（线性于序列长度）。在达到**同等有效感受野/同等长程关联**的前提下，要用二次复杂度的 softmax 去匹配线性复杂度旋度所覆盖的长程依赖，所需的等效宽度（参数量 $\propto d^2$）随目标关联长度 $\ell$ 的增长而放大。设目标关联长度对应的有效 token 数为 $m$，则匹配同等长程能力所需的参数比量级为下式，标记 (C14.9)：

$$
R_{\mathrm{cplx}} \;\sim\; \frac{m^2}{m} = m
$$

在典型语言任务中，真正起作用的中程依赖窗口 $m$ 在数倍量级（经验上 $m\sim 5$–$10$，对应一个句子到一个段落的关键依赖跨度）。这给出区间上端约 **5–10 倍**。须注意此论证依赖"目标关联长度"这一任务相关量，故其上端随任务而变——这正是 C14.4 须用多尺度等算力曲线（测试二）实测的原因。

#### C14.2-bis.3 论证三（信息容量）：预测信息的乘性叠加

由命题 C3.3，旋度直接贡献非零预测信息 $I_{\mathrm{pred}}$，且由 (C3.14) 其下界正比于稳态熵产生率 $\sigma$。在纯梯度流中 $I_{\mathrm{pred}}=0$，要靠参数堆叠去**逼近**等量的预测能力。设单层旋度贡献的预测信息为 $\iota$，纯梯度流用 $p$ 个参数才能逼近同等 $\iota$；在 $L$ 层网络中，若各层旋度近似独立贡献，则总预测信息按层数线性累积，而纯梯度流为匹配之所需参数按**乘性**放大（每层都要补一组参数去模拟该层缺失的非保守流）。于是参数效率比的量级为下式，标记 (C14.10)：

$$
R_{\mathrm{info}} \;\sim\; R_{\mathrm{dof}}^{(1)} \times f(L) \;\approx\; 2 \times f(L)
$$

其中 $R_{\mathrm{dof}}^{(1)}\approx 2$ 为单层下界 (C14.8)，$f(L)$ 为深度叠加因子。对实践中常见的深度（$L\sim$ 数十层），在收益随深度饱和的保守估计下 $f(L)\in[1.5, 5]$，故 $R_{\mathrm{info}}\in[3, 10]$。

#### C14.2-bis.4 三条论证的收敛与区间的确定

三条独立论证给出的量级汇总于下表：

| 论证 | 角度 | 给出的量级 | 对应区间端 |
|---|---|---|---|
| C14.2-bis.1 | 自由度计数 | $2d/(d+1)\to 2$（单层），深度累积上推 | 下端（约 3×） |
| C14.2-bis.2 | 复杂度类 | $\sim m$（中程窗口 $m\sim 5$–$10$） | 上端（约 5–10×） |
| C14.2-bis.3 | 信息容量 | $2\times f(L)\in[3,10]$ | 全区间 |

三条从不同物理原理出发的论证**收敛到同一量级区间 [3, 10]**，本文据此把可证伪承诺取为其稳健交集，即下式，标记 (C14.11)：

$$
R_{\mathrm{param}} \;=\; \frac{\text{基线参数量}}{\text{CID 参数量}}\Big|_{\text{同等性能}} \;\approx\; 5\text{–}10
$$

**边界与诚实声明**：(C14.11) 是**量级估计**而非定理，其三条论证各依赖一项可证伪假设——论证一假设 $\Omega_A$ 可完全由温差派生而零新增参数；论证二假设任务的有效中程窗口 $m\sim 5$–$10$；论证三假设各层旋度贡献近似独立且深度叠加因子 $f(L)$ 不退化。任一假设若在实测中显著破裂，区间即随之移动。因此本文不把 5–10 倍当作已证结论，而是把它写成 C14.4 测试二的**判决性证伪点**：若多尺度等算力曲线给出的 $R_{\mathrm{param}}$ 显著低于 5×（如 C14.4 证伪条件所列），则本承诺被证伪。Phase 1 在单一 10M 尺度上测得的 3.22× 等参数损失比与本区间下端方向一致，但（如摘要所诚实标注）尚不构成对 (C14.11) 的直接检验——直接检验须多尺度标度曲线，列为完整 Phase 1 待办。
