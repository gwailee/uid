...
**可证伪预言与初步实证**：本文据此提出**约 5 至 10 倍参数效率**的可证伪工程目标，并给出三组**已被生物大脑独立实证**的临界普适类预言：雪崩规模指数 τ ≈ 1.5（[Beggs & Plenz，2003](https://doi.org/10.1523/JNEUROSCI.23-35-11167.2003)）、Hurst 指数 H ≈ 0.7（[Linkenkaer-Hansen 等，2001](https://doi.org/10.1523/JNEUROSCI.21-04-01370.2001)）、1/f 噪声谱斜率 β ≈ 1（[He，2014](https://doi.org/10.1016/j.tics.2014.04.003)）。**本版本（v2.3）完成 Phase 1 多尺度实证**（10M/30M/100M 三尺度、中文 MiniMind 语料、3 个随机种子；含 11 路消融的三尺度扫描、临界指数与解码能耗，以及一份第三方独立复现；完整报告与全部原始数据见配套仓库）。v2.3 相对早期版本的三项关键更新：将记忆核长度由 64 提升至 512；修正测量工具链的三处缺陷（见下文（三））；并**确认 §8.5 借用的 ET 对称项在因果语言模型中本质非因果、不可实现**（见下文关于 ET 的说明）。

**（一）框架优势（T1）获得强支持，且随规模单调放大。** 在与标准注意力骨架完全相同的前提下仅装入 UID 的三个**因果安全**物理项（旋度、色阻尼记忆核、OU 色噪声）。在 10M/30M/100M 三尺度消融（memory_length=512，3 种子）中，`cid_full` 相对纯 Transformer 的困惑度比**随规模单调放大**：10M 为 73.58/21.95 = **3.35 倍**，30M 为 46.36/12.49 = **3.71 倍**，100M 为 41.76/10.21 = **4.09 倍**。尤为关键的是：**`cid_full` 在 10M（约 4.8M 非嵌入参数，困惑度 21.95）已显著优于纯 Transformer 在 100M（约 49M 参数，困惑度 41.76）——即 CID 以约 1/10 的参数即胜出**，方向性地支持 5–10 倍参数效率（T2）的目标。作为对照，充分训练（约 200 token/参数，memory_length=64）的 10M 单点为 `cid_full` 7.90 vs Transformer 31.12（3.94 倍）。色阻尼记忆核是单项贡献最大的物理项（各尺度移除均使困惑度上升 15–22%），且物理 OU 噪声优于 FFT 谱整形（幅度随规模变化：10M 约 7 倍、100M 约 1.24 倍），与第一部分第 14.2 节一致。

**（二）"注意力并不够"在三个尺度与充分训练设置中一致复现。** 已知 Transformer 技巧（深度卷积、线性对易子、噪声注入）在 10M/30M/100M 每个尺度的增益均 < 1%；在充分训练的缩放律设置中 `transformer_plus_tricks`（31.23）甚至略差于纯 Transformer（31.12），却多耗约 2.3 倍算力。CID 的优势因此不能归因于"堆叠更多技巧"，而来自架构层面的物理重构。

**（三）临界普适类预言获部分支持，且如理论所预期无法区分基线。** 在修正测量工具后（v2.3 修复三处测量缺陷；以 shuffle 替代对照 Hurst = 0.519 ≈ 0.5 验证估计器无偏，谱拟合 R² = 0.94）：Hurst 指数 H = 0.803 落入预言区间 [0.6, 0.8] 且远高于替代对照（**F4 通过**），表明隐状态沿 token 轴存在真实长程正相关；而 1/f 谱斜率 β = 0.572 略低于 [0.7, 1.3]（**F3 未达**），雪崩尾部未通过幂律检验（**F5 未达**，附测量有效性说明），各向异性 η = 0.997 虽 > 0.5 但与 Transformer 基线（0.998）几乎相同而**无区分力**（**F6 不确定**）。**关键的诚实结论是：四个临界指数均无法把 CID 与普通 Transformer 区分开**——这恰好印证本文摘要早已申明的立场：这些普适指数区间相当宽、可证伪强度有限，能排除白噪声等平凡情形，却难以把 CID 与其他同样表现出自组织临界的模型区分开；它们仅作为"CID 表现出与生物大脑一致的临界统计特征"的描述性旁证，**而非** CID 优于 Transformer 的独立证据。

**（四）能耗在修正测量后可干净测得，但其判决性检验须俟多尺度。** 在稳健的全局空闲基线下（61.9 W，窗口间离散 0.06 W；修复了此前各家族空闲功耗相差约 87 W 的测量伪差异），等参数解码下 `cid_full` 的每 token 超空闲能耗（0.160 mJ）仅比 Transformer（0.141 mJ）高约 **13%**，且低于 `transformer_plus_tricks`（0.164 mJ），同时参数最少、峰值功率最低——结合约 3.9 倍困惑度优势，即"以约 13% 的能耗代价换取约 3.9 倍质量"。

**须诚实标注全部边界**：上述三尺度消融的优势比值（3.35–4.09 倍）为**消融预算（单轮训练）下的方向性下界**，13% 是**等参数能耗开销**，二者均与 5–10 倍参数效率（T2）及 ≥3 倍能效（C13）的目标方向一致，但**都不直接检验之**；T2 须多尺度等算力标度曲线、C13 须等性能（iso-PPL）能耗曲线，两者在单一尺度下均无法成立（该尺度下 Transformer 无法达到 CID 的困惑度，故等性能比当前仅为下界），列为完整 Phase 1（1b，H100）的判决性待办。此外，关于借用自 [Hoover 等（2023）](https://arxiv.org/abs/2302.07253)的 ET 对称项（预注册条件 F8），v2.3 给出一个**结构性结论**：ET 能量函数负梯度的对称第二项要求 key 位置依赖未来的 query 位置，在自回归因果语言模型中会造成未来 token 泄漏（因果回归测试实测约 0.11），即 **ET 的双向对称性与自回归因果性数学上不可兼容**。因此在因果 LM 设置下该项被舍弃、ET 退化为标准因果注意力，`cid_full` 与 `cid_full_no_et` 数值等价。这一结论**取代**早期版本"F8 判为 FAIL"的表述：正确的结论不是"ET 无用"，而是 **ET 不适用于因果语言模型**；相应地，CID 的全部优势来自其三个**因果安全**的物理项，与借用的 ET 组件无关（详见第一部分第 16 章的初步实证小节）。综上，**真正具有区分力的判决点是参数效率承诺与关联长度标度**（临界指数本身区分力不足这一点，已被本轮实测进一步证实）。UID 的参数效率预言与 Alman-Song-Gupta 复杂度下界**互补而非冲突**——前者通过脱离 softmax-attention 接口、进入不同复杂度类而获得收益。
...
本文所有参考文献均提供可点击、可达第一手来源的 DOI 或开放访问超链接，所有定量声明明确标注实证等级（A 已验证 / B 理论严格待实证 / C 可证伪工程目标 / D 哲学猜想）。配套代码仓库（[github.com/gwailee/uid](https://github.com/gwailee/uid)）提供 CID 的工程参考实现与可证伪验证套件，所有核心预言可在单卡 GPU 上数小时内复现。**本版本（v2.3）随附 Phase 1 多尺度实证的完整报告（含 11 路变体在 10M/30M/100M 三尺度 × 3 种子的全部逐种子记录、提交哈希、数据 SHA-256、负面结果，以及一份第三方独立复现），遵循"无选择性报告、无事后调整、负面结果同等显著呈现"的承诺；其中 F3（β）、F5（雪崩）的 FAIL、F6（η）的无区分力、以及 F8（ET 项因非因果而不可实现）均与被支持的 T1 趋势、F4（Hurst）、OU 优于 FFT 等以同等篇幅并列报告。v2.3 相对早期版本的工程改动（记忆核 64→512、ET 舍弃、临界指数工具链修正）均如实记录，改动前后结果一并保留、不作替换。须重申：多尺度消融的优势比值为消融预算下的方向性下界，T2 与 C13 的判决性检验须在等算力多尺度标度曲线与等性能能耗曲线上完成，列为 Phase 1b（H100）待办。引用本版本实验结果时须附 v2.3 提交哈希及报告第 6 节列出的逐项注意事项。**
...
本文提出统一智动力学（Unified Intelligo-Dynamics, UID）框架，主张智能系统的演化可统一描述为信息几何流形上的非平衡随机场动力学。该框架包含三个嵌套层级——经典层（CID）、量子层（QID）、场几何层（FID），以及向多智能体系统的群体推广。本文第一部分聚焦经典层 CID，作出三点贡献。
...
graph TD
    ...
    F["所有主流架构都是它的特解<br/>(共同缺陷：旋度 v ≡ 0)"]
    G1["Transformer"]
    G2["Mamba"]
    G3["Diffusion"]
    G4["World Model"]
    G5["JEPA"]
    G6["Reasoning (R1/o 系列)"]
    G7["MoE 稀疏路由"]
    H["可证伪预言"]
    I1["Hurst≈0.7"]
    I2["τ≈1.5"]
    I3["约 5 至 10 倍效率<br/>(量级来源见 C14.2-bis)"]
...
| **世界模型** | **0** | **指数衰减（欧姆）** | **单尺度白噪声** | **旋度关闭、记忆+噪声** |
| **JEPA** | **0** | **δ 函数** | **0** | **全部三项关闭（隐空间势）** |
| **推理增强（R1/o-系列）** | **0** | **δ 函数** | **0** | **单步全关闭 + 离散外循环** |
| **稀疏路由（MoE）** | **0** | **δ 函数** | **0** | **全部三项关闭（势能分块）** |
...
#### C 第 6.4-bis 节：世界模型、JEPA、推理增强与稀疏路由——同一极限族内的进一步特解

C6.2–C6.4 已严格归约 Transformer、Mamba、扩散模型三个最基础的架构。本节按完全相同的"逐项取极限"策略（开关 C6.1–C6.3），把摘要中列举但尚未在正文展开的四类近期架构——**世界模型**（World Models）、**JEPA**、**推理增强模型**（DeepSeek-R1、o1–o3）、**稀疏路由架构**（MoE）——逐一定位到 CID 极限坐标上。本节的目的不是宣称这些架构的提出者做错了什么，而是证明：尽管它们在工程动机上各异，**其动力学骨架仍落在 v ≡ 0 的同一特解族内**，因而共享 C6.5 将指出的同一根本缺陷。须先行声明归属边界：本节不主张这些架构的任何单项设计为本文首创，仅在 CID 极限坐标上标出其位置与所砍掉的物理项。

#### C6.4-bis 世界模型、JEPA、推理增强与稀疏路由——同一极限族内的进一步特解

C6.2–C6.4 已严格归约 Transformer、Mamba、扩散模型三个最基础的架构。本节按完全相同的"逐项取极限"策略，把世界模型、JEPA、推理增强模型（DeepSeek-R1、o1–o3）、稀疏路由架构（MoE）逐一定位到 CID 极限坐标上。本节不主张这些架构的任何单项设计为本文首创，仅在 CID 极限坐标上标出其位置与所砍掉的物理项。

##### C6.4-bis.1 世界模型

世界模型（[Ha & Schmidhuber, 2018](https://arxiv.org/abs/1803.10122)；[Hafner 等, 2023](https://arxiv.org/abs/2301.04104)）的核心是在隐空间 $z$ 上学习的状态转移分布 $p( z_{t+1} \mid z_t , a_t )$ 。在 CID 中，这对应令旋度 $v \equiv 0$ 、取指数（欧姆）记忆核、噪声取单尺度高斯、势能取由重构损失定义的二次型。其隐空间演化为式 (C6.20a)：

$$
\frac{dz}{dt} = -\nabla U(z) - \int_0^t \gamma_0 \, e^{-(t-s)/\tau_c} \, \dot z(s) \, ds + \xi_{\mathrm{white}}(t) , \qquad v \equiv 0
$$

世界模型同时保留短记忆与白噪声两项，却仍关闭旋度，是三大基础架构中最接近完整 CID 的一个，但仍缺最关键的旋度。其位置为式 (C6.20b)：

$$
\mathrm{WorldModel} = \mathrm{CID} \mid \left( v = 0 ,\ \text{指数记忆} ,\ \text{单尺度白噪声} \right)
$$

##### C6.4-bis.2 JEPA

JEPA（[LeCun, 2022](https://openreview.net/forum?id=BZ5a1r-kVsf)；[Assran 等, 2023](https://arxiv.org/abs/2301.08243)）在隐空间预测目标表征，以能量函数度量预测嵌入与目标嵌入的失配。在 CID 中，这对应令 $v \equiv 0$ 、 $\gamma \to \delta$ 核、 $\xi \to 0$ ，得纯梯度流，为式 (C6.20c)：

$$
\frac{dz}{dt} = -\nabla U_{\mathrm{JEPA}}(z) , \qquad v \equiv 0 ,\ \xi \equiv 0
$$

JEPA 与 Transformer 在动力学骨架上完全同构，区别仅在势能的具体形式。其位置为式 (C6.20d)：

$$
\mathrm{JEPA} = \mathrm{CID} \mid \left( v = 0 ,\ \delta\ \text{记忆} ,\ \text{无噪声} \right)
$$

##### C6.4-bis.3 推理增强模型

推理增强模型（DeepSeek-R1、o1–o3）每一步前向计算仍是标准 Transformer，即 CID 在 $v \equiv 0$ 下的特解；多步推理是叠加在该单步特解之外的离散外循环 $z^{(k+1)} = G( z^{(k)} )$ 。离散外循环的复合 $G^{\circ k}$ 仍是保守映射的复合，不产生连续动力学层的稳态环流。其位置为式 (C6.20e)：

$$
\mathrm{ReasoningModel} = \left[ \, \mathrm{CID} \mid \left( v = 0 ,\ \delta\ \text{记忆} ,\ \text{无噪声} \right) \right]^{\circ k}
$$

##### C6.4-bis.4 稀疏路由 MoE

MoE（[Shazeer 等, 2017](https://arxiv.org/abs/1701.06538)）由门控网络在多个专家间稀疏选择，对应把势能取为分块结构 $U( \varphi ) = \sum_e g_e ( \varphi ) \, U_e ( \varphi )$ 。无论门控如何切换，加权总流仍是保守场，未引入旋度。其位置为式 (C6.20f)：

$$
\mathrm{MoE} = \mathrm{CID} \mid \left( v = 0 ,\ \delta\ \text{记忆} ,\ U = \sum_e g_e \, U_e \right)
$$

#### C6.5 统一图景：七类架构、七处取舍、一个共同缺陷

综合 C6.2–C6.4 与 C6.4-bis，当前几乎全部主流深度学习架构在 CID 极限坐标下的位置一目了然。

Transformer 的位置为式 (C6.18)：

$$
\mathrm{Transformer} = \mathrm{CID} \mid \left( v = 0 ,\ \delta\ \text{记忆} ,\ \text{无噪声} \right)
$$

Mamba 的位置为式 (C6.19)：

$$
\mathrm{Mamba} = \mathrm{CID} \mid \left( v = 0 ,\ \text{指数记忆} ,\ \text{单尺度噪声} \right)
$$

扩散模型的位置为式 (C6.20)：

$$
\mathrm{DiffusionModel} = \mathrm{CID} \mid \left( v = 0 ,\ \delta\ \text{记忆} ,\ \text{白噪声} \right)
$$

七类架构在三个开关（旋度 $v$ 、记忆核 $\gamma$ 、噪声 $\xi$ ）下的取舍各异，但共同缺陷清晰可见：旋度项 $v$ 一律被设为零。由命题 C3.2， $v \equiv 0$ 意味着系统满足细致平衡；由命题 C3.3，细致平衡意味着零内禀预测信息。这就是"注意力并不够"的完整物理论证链，为式 (C6.21)：

$$
\text{softmax 注意力} \subset \text{纯保守流} \ ( v \equiv 0 ) \;\Rightarrow\; \text{细致平衡} \;\Rightarrow\; \sigma = 0 \;\Rightarrow\; I_{\mathrm{pred}} = 0
$$

链条 (C6.21) 的前提已由 C6.4-bis 从 softmax 注意力放宽为任何 $v \equiv 0$ 的纯保守流，因而同样适用于其余六类架构。突破这一上限的唯一出路，是重新装回 CID 被砍掉的三项——首先是旋度 $v ( \varphi )$ 。其可工程化实现与 5–10 倍参数效率的量级来源见第 14 章。
...
#### C14.2-bis 5–10 倍区间的量级推导

本节给出 5–10 倍承诺的量级来源。这是一个在明确假设下的量级估计，而非从公理严格导出的定理；其最终裁决交由 C14.4 的判决性实验（测试二）。

##### C14.2-bis.1 论证一（自由度计数）

在 $d$ 维隐空间中，保守梯度流的稳态由对称正定矩阵刻画，独立自由度为 $d ( d + 1 ) / 2$ ；装回旋度后额外获得反对称部分的自由度 $d ( d - 1 ) / 2$ ，且由零参数旋度机制不需新增可训练参数。相同参数预算下的有效自由度提升比为式 (C14.8)：

$$
R_{\mathrm{dof}} = \frac{ \dfrac{d ( d + 1 )}{2} + \dfrac{d ( d - 1 )}{2} }{ \dfrac{d ( d + 1 )}{2} } = \frac{2d}{d + 1} \;\xrightarrow{\ d \gg 1\ }\; 2
$$

这给出单层约 2 倍的下界，深度累积后落在 2–3 倍之上。

##### C14.2-bis.2 论证二（复杂度类）

softmax 注意力在序列长度 $n$ 上有 $\Omega ( n^2 )$ 复杂度下界，旋度的非保守环流复杂度为 $O ( n \, d )$ 。设目标关联长度对应有效 token 数为 $m$ ，匹配同等长程能力所需参数比量级为式 (C14.9)：

$$
R_{\mathrm{cplx}} \;\sim\; \frac{m^2}{m} = m
$$

典型语言任务中起作用的中程窗口 $m \sim 5\text{–}10$ ，这给出区间上端约 5–10 倍。

##### C14.2-bis.3 论证三（信息容量）

由命题 C3.3，旋度直接贡献非零预测信息，纯梯度流须靠参数堆叠逼近。在 $L$ 层网络中，总预测信息按层线性累积，纯梯度流为匹配之所需参数按乘性放大。参数效率比量级为式 (C14.10)：

$$
R_{\mathrm{info}} \;\sim\; R_{\mathrm{dof}}^{(1)} \times f(L) \;\approx\; 2 \times f(L)
$$

其中 $f(L) \in [ 1.5 , 5 ]$ ，故 $R_{\mathrm{info}} \in [ 3 , 10 ]$ 。

##### C14.2-bis.4 三条论证的收敛

三条独立论证收敛到同一量级区间 $[ 3 , 10 ]$ 。本文取其稳健交集作为可证伪承诺，为式 (C14.11)：

$$
R_{\mathrm{param}} = \left. \frac{ \text{基线参数量} }{ \text{CID 参数量} } \right|_{\text{同等性能}} \approx 5\text{–}10
$$

须诚实声明：(C14.11) 是量级估计而非定理，三条论证各依赖一项可证伪假设。若 C14.4 测试二的多尺度等算力曲线给出的 $R_{\mathrm{param}}$ 显著低于 5 倍，则本承诺被证伪。
...
### C16 Phase 1 初步实证：多尺度消融、临界指数与能耗（v2.3）

本节报告 UID v2.3 随附的 Phase 1 初步实证。实验在中文 MiniMind 预训练语料（10 万条子集，约 1000 万 token）上、3 个随机种子（[42, 43, 44]）、单张 NVIDIA RTX 4090 上完成，涵盖四类实验：**(1) 11 路组件消融的 10M/30M/100M 三尺度扫描、(2) 三家族缩放律家族训练（10M）、(3) 临界指数测量、(4) 解码能耗测量**。完整报告、逐种子原始记录、提交哈希、数据 SHA-256 与一份第三方独立复现见配套仓库 [`results/phase1/REPORT.md`](https://github.com/gwailee/uid)。

须在开篇即明确本节结论的**双重边界**，以免任何过度解读：

- 本节多尺度消融给出的优势比值（10M 3.35 倍、30M 3.71 倍、100M 4.09 倍）均为**消融预算（单轮训练）下的等参数困惑度比**，是参数效率的**方向性下界**，**不是**严格的参数效率测量；理论的 5–10 倍参数效率承诺（T2）须由多尺度**等算力**标度曲线检验，列为完整 Phase 1（1b，H100）待办。
- 能耗结果为**等参数（iso-parameter）开销**，**不是** C13 的等性能（iso-PPL）能效判决；后者在单一尺度下无法成立，亦列为 Phase 1b 待办。

我们以"无选择性报告、无事后调整、负面结果同等篇幅呈现"的承诺报告全部结果，包括两项 FAIL、一项无区分力的 INCONCLUSIVE，以及对 ET 项的结构性否证。v2.3 相对早期版本的三项工程改动（记忆核长度 64→512、ET 对称项因非因果而舍弃、临界指数工具链修正）均如实记录，改动前后结果一并保留、不作替换（见 16.8）。

#### C16.1 框架优势（T1）：随规模单调放大，CID 以约 1/10 参数胜出

在与标准注意力骨架完全相同的前提下，仅装入 UID 的三个**因果安全**物理项（旋度 $v(\varphi)$、色阻尼记忆核 $\int\gamma$、OU 色噪声 $\xi$）。10M/30M/100M 三尺度消融（memory_length=512，1 epoch，3 种子）结果如下：

| 尺度 | `cid_full` 非嵌入参数 | `cid_full` 困惑度 | `transformer` 困惑度 | 困惑度比 |
|---|---|---|---|---|
| 10M | 约 4.8M | **21.95**（21.78/21.95/22.12）| 73.58 | **3.35 倍** |
| 30M | 约 22.7M | **12.49**（12.51/12.48/12.47）| 46.36 | **3.71 倍** |
| 100M | 约 49.3M | **10.21**（10.26/10.21/10.17）| 41.76 | **4.09 倍** |

三点值得强调：

1. **优势随规模单调放大**（3.35 → 3.71 → 4.09 倍）。架构优势"越大越强"，与"基线经更充分训练/更大规模即可追平"这一典型失效模式**相反**——后者是许多架构改进最终失效的方式，而此处恰恰相反。
2. **CID 以约 1/10 的参数胜出**：`cid_full` 在 10M（约 4.8M 非嵌入参数、困惑度 21.95）已显著优于纯 `transformer` 在 100M（约 49M 参数、困惑度 41.76）。这是参数效率的一个**强下界**信号，方向上与 5–10 倍参数效率（T2）目标一致。
3. **跨种子近零方差**（如 100M 的 `cid_full` 标准差约 0.05），表明这是稳定的架构性差异，而非幸运种子。

作为**充分训练对照**，三家族缩放律训练（约 200 token/参数，memory_length=64，10M，3 种子）给出：`transformer` 困惑度 31.12、`transformer_plus_tricks` 31.23、`cid_full` **7.90**（7.88/7.90/7.90，标准差 ≈ 0.01），即充分训练下 10M 单点比值为 **3.94 倍**，且 CID 训练损失（2.355）亦远低于 Transformer（3.345），排除"CID 仅靠正则化取胜"的解释。

（口径提示：三尺度消融为单轮训练、memory_length=512；缩放律对照为充分训练、memory_length=64。两套预算不同、不可直接连成一条标度曲线；严格 T2 判决须等算力多尺度曲线，见 16.8 caveat C1。）

#### C16.2 "注意力并不够"：跨三尺度与充分训练一致复现

已知 Transformer 技巧在所有设置中均无效：

- **三尺度消融**：五个 Transformer 变体（baseline、+conv、+linear、+noise、+all_tricks）在 10M/30M/100M 每个尺度的困惑度差异均 < 1%。
- **充分训练缩放律**：`transformer_plus_tricks`（31.23）甚至略差于纯 `transformer`（31.12），却多耗约 **2.3 倍**算力（wall-clock）。
- **能耗**（见 16.5）：`transformer_plus_tricks` 消耗最高的每 token 能耗，换来最差的质量。

因此，CID 的优势**不能归因于"堆叠更多技巧"**，而来自架构层面的物理重构——与本文"注意力并不够"的核心论断一致。

#### C16.3 各物理项贡献与 ET 项的结构性结论

各物理项的单项贡献（相对 `cid_full`，三尺度一致）：

| 切除的组件 | 困惑度变化（10M / 30M / 100M）| 解读 |
|---|---|---|
| 色阻尼记忆核（$\int\gamma$） | +21.7% / +14.7% / +14.8% | **单项贡献最大的物理项**，跨尺度稳定；对应 Transformer 砍掉的色阻尼项 |
| 旋度（$v(\varphi)$） | +0.2 ~ +0.4% | 消融效应小；其*必要性*应由临界指数检验（命题 C3.3），见 16.4 |
| 色噪声（存在性） | ≈ 0% | 存在性影响小；关键在噪声*类型*（OU vs FFT）而非存在 |
| OU → FFT 噪声 | 约 7 倍（10M）/ 约 1.24 倍（100M）更差 | OU 优于 FFT，支持 §14.2；但优势幅度随规模收窄 |

两点须诚实标注：

- **记忆核为主导物理项**，其相对贡献在三个尺度稳定在 15–22%，直接对应理论所述 Transformer 砍掉的 $\int\gamma$ 色阻尼项。将记忆核长度由 64 提升至 512（v2.3 改动）使 `cid_full` 在 10M 的困惑度由 23.62（旧，L=64）降至 21.95（L=512），改善约 7%；改动前后结果均保留（见 16.8）。
- **OU 优于 FFT 的幅度随规模显著变化**：10M 时 FFT 噪声灾难性劣化（困惑度约 157，为全套件最差且训练不稳定），100M 时 FFT 仅温和更差（约 12.67）。这说明 10M 的极端劣化部分源于小规模训练不稳定；OU 为物理默认在各尺度均获支持，但"约 7 倍"这一幅度是 10M 特定值，须分尺度报告。

**关于 ET 对称项（原预注册条件 F8）的 v2.3 结论**：早期版本曾将"启用 ET 对称项"作为一个消融变体，并报告其无正面收益（F8 FAIL）。v2.3 给出一个更根本的**结构性结论**：ET 能量函数 $E_{\text{ATT}}(g)=-\tfrac1\beta\sum_C\log\sum_{B\neq C}\exp(\beta K_B\!\cdot\!Q_C)$ 的负梯度含两项，其**对称第二项**要求 key 位置 $B$ 的更新聚合 query 位置 $C$（且唯一自洽方向为 $C\ge B$），即**依赖未来 token**；在自回归因果语言模型中，因果回归测试实测该项造成约 0.11 的未来 token 泄漏。**ET 的双向对称性与自回归因果性数学上不可兼容**——Hoover 等（2023）的 ET 与其 Lyapunov 单调下降证明成立于**非因果**联想记忆设置，而非因果 LM。因此 v2.3 在因果 LM 参考实现中**舍弃该项**，`use_et_symmetric=True` 退化为标准因果注意力，`cid_full` 与 `cid_full_no_et` 数值等价（三尺度均逐位一致）。

这一结论**取代**早期"F8 判为 FAIL"的表述。正确的读法不是"ET 无用"，而是 **ET 不适用于因果语言模型**；相应地，CID 的全部优势来自其三个**因果安全**的物理项（旋度、色阻尼记忆核、色噪声），与借用自 Hoover 等（2023）的、非本文首创的 ET 组件无关。此结构性结论"提纯"了优势归属，且与本文关于 ET 归属的一贯声明（ET 非 UID 原创）完全一致。

#### C16.4 临界指数（10M）：部分支持，且无法区分基线

临界指数测量在训练好的 `cid_full_10M` 与 `transformer_10M` 检查点上、噪声注入关闭、2000 序列 × 512 token 上完成。**v2.3 修正了测量工具链的三处缺陷**（见 caveat C5），修正后以 **shuffle 替代对照的 Hurst = 0.519 ≈ 0.5**（白噪声值）与**谱拟合 $R^2 = 0.94$** 验证估计器已无偏。修正后结果：

| 指标（预注册条件） | UID 预言 | `cid_full` | shuffle 替代对照 | Transformer 基线 | 判定 |
|---|---|---|---|---|---|
| Hurst（DFA-2，**F4**） | [0.6, 0.8] | **0.803** | 0.519 | 0.813 | ✅ **PASS** |
| 1/f 谱斜率 β（**F3**） | [0.7, 1.3] | **0.572**（$R^2$=0.94） | ≈0（$R^2\approx0$） | 0.709 | ❌ FAIL |
| 雪崩指数 τ（**F5**） | ~1.5 | 非幂律（α≈3.0, KS p=0） | — | 非幂律 | ❌ FAIL |
| Fisher 各向异性 η（**F6**） | > 0.5 | 0.997 | — | 0.998 | ⚠️ 无区分力 |

- **F4（Hurst）通过**：H = 0.803 落入预言区间，且**远高于 shuffle 替代对照（0.519）**，表明隐状态沿 token 轴存在真实长程正相关，与人类皮层观测值（[Linkenkaer-Hansen 等，2001](https://doi.org/10.1523/JNEUROSCI.21-04-01370.2001)）方向一致。
- **F3（β）未达**：β = 0.572 略低于 [0.7, 1.3] 下界；Transformer 基线（0.709）反而更接近预言。
- **F5（雪崩）未达**：两模型尾部均未通过 Clauset 幂律检验（KS p = 0），α ≈ 3.0 远离 τ ≈ 1.5；附 caveat C5 中的测量有效性说明（`xmin` 异常偏大）。
- **F6（η）不确定**：η = 0.997 > 0.5，但与 Transformer 基线（0.998）几乎相同——度量 $\eta=(\lambda_{\max}-\lambda_{\min})/(\lambda_{\max}+\lambda_{\min})$ 对重尾协方差谱饱和趋于 1，**无区分力**，故判 INCONCLUSIVE 而非 PASS（度量重构建议见 caveat C6）。

**本节最重要的诚实结论**：四个临界指数**均无法把 CID 与普通 Transformer 区分开**（H 0.80 vs 0.81；β 0.57 vs 0.71；η 0.997 vs 0.998）。这恰好印证本文摘要早已申明的立场——这三组普适指数预言区间相当宽、可证伪强度有限，能排除白噪声等平凡情形，却难以把 CID 与其他同样表现出自组织临界的模型区分开。因此本文**不将临界指数作为 CID 优于 Transformer 的区分性证据**，仅作为"CID 表现出与生物大脑一致的临界统计特征"的描述性旁证；真正具有区分力的判决点是参数效率承诺与关联长度标度（见 16.6）。

#### C16.5 解码能耗（10M）：等参数开销

能耗在解码模式（每次生成 64 token，batch 8 × seq_len 256）下、以 pynvml @ 25 Hz 高频采样测量。**v2.3 采用稳健的全局空闲基线**（3 窗口中位数 = 61.9 W，窗口间离散仅 0.06 W），修复了早期各家族空闲功耗相差约 87 W（CID 124 W vs Transformer 211 W）导致 above-idle 列不可比的测量伪差异（见 caveat C5）。

| 家族 | 困惑度 | 平均功率 (W) | wall-clock (s) | 超空闲能耗 (mJ/token) | 相对基线 |
|---|---|---|---|---|---|
| `transformer` | 31.12 | 265.1 | 177.6 | **0.141** | 1.00× |
| **`cid_full`** | **7.90** | 251.4 | 215.9 | 0.160 | **1.13×（开销）** |
| `transformer_plus_tricks` | 31.23 | 227.1 | 254.6 | 0.164 | 1.17× |

等参数下，`cid_full` 每 token 超空闲能耗仅比 Transformer 高约 **13%**，且**低于** `transformer_plus_tricks`（后者以最高能耗换最差质量），同时 CID 参数最少、峰值功率最低（259.9 W vs Transformer 275.1 W）——其开销主要来自串行延迟而非峰值功率，对工程优化更友好。结合约 3.9 倍困惑度优势，可概括为：**CID 以约 13% 的每 token 能耗代价，换取约 3.9 倍的困惑度优势**。

须再次强调：上述为**等参数开销**，**不是** C13 的"等性能下 ≥3 倍能效"判决（预注册条件 F7）。C13 须在等性能（iso-PPL）下测量——即"达到 CID 困惑度所需的 Transformer 耗多少能"，而单一尺度的 Transformer 根本无法达到该困惑度，故 iso-PPL 能耗比当前未定义（测量工具正确地拒绝外推）。F7 须俟多尺度曲线，列为 Phase 1b 待办。

#### C16.6 Phase 1 证伪记分牌

| 判定 | 预注册条件 | 说明 |
|---|---|---|
| ✅ PASS（1） | F4（Hurst） | H = 0.803 ∈ [0.6,0.8]，surrogate 验证为真长程相关 |
| ❌ FAIL（2） | F3（β）、F5（雪崩） | β=0.572 略低；雪崩非幂律（附测量有效性 caveat）|
| ⚠️ INCONCLUSIVE（1） | F6（η） | η>0.5 但与基线无区分，度量饱和 |
| 🔧 RESOLVED_not_realizable（1） | F8（ET 对称项） | ET 非因果、因果 LM 中不可实现，已舍弃；非"无用"而是"不适用" |
| ⏳ ABSTAIN（3） | F1、F2（等算力多尺度曲线）、F7（单尺度 iso-PPL 无法测） | T2 与 C13 的**判决点**，列入 Phase 1b（H100）|

须特别指出：F3/F5 的 FAIL 与 F6 的无区分力，均涉及**B 级（理论严格、待实证）的临界普适类预言**，本文已申明这些预言区分力有限、非核心判决点；而 F8 的 RESOLVED 是一个**结构性**结论（ET 与因果性不可兼容），仅涉及借自 Hoover 等（2023）的非首创组件，不触及 UID 自身的三个物理项。**核心判决点 T2（参数效率）与 C13（能效）尚未被判决性检验**——本节全部正面结果（T1 随规模放大、Hurst、OU>FFT）与全部负面结果（β、雪崩、η）均不改变这一待办状态；但三尺度消融中"优势随规模放大、CID 以约 1/10 参数胜出"已为 T2 提供了强方向性下界。

#### C16.7 与理论命题的对应

| 理论命题 | 是否 UID 原创 | 本轮判定 | 正确检验手段 |
|---|---|---|---|
| **T1**：CID 物理项 > Transformer | 是 | **支持，且随规模放大**（3.35→3.71→4.09 倍；约 1/10 参数胜出）| 多尺度消融（16.1）+ 缩放律 |
| **T2**：5–10× 参数效率 | 是 | 方向性强支持（消融预算下下界约 10 倍参数）；判决待定 | 等算力多尺度曲线（F1/F2），Phase 1b |
| **C3.3 / 旋度必要性**（非平衡） | 是 | 部分：Hurst 通过；β/雪崩未达；无基线区分 | 临界指数（F3–F6） |
| **§14.2**：OU > FFT 色噪声 | 是 | **各尺度支持**（幅度随规模变化：10M~7×、100M~1.24×）| 消融（16.3） |
| **色阻尼记忆核（$\int\gamma$）** | 是 | **支持，主导项**（各尺度移除 +15~22%）| 消融（16.3） |
| **C13**：iso-PPL 下 ≥3× 能效（F7） | 是 | 单尺度无法检验（iso-param 开销 1.13×）| 多尺度 iso-PPL（F7），Phase 1b |
| **ET 对称项（§8.5）** | **否**（Hoover 2023） | **RESOLVED：非因果、因果 LM 不可实现，已舍弃** | 因果回归测试（caveat C4）|

### C16.8 局限与逐条注意事项（caveat）

引用本节任何结果时，须同时引用以下逐条注意事项：

- **C1（消融预算 vs 等算力）**：C16.1 的三尺度趋势用的是消融预算（单轮训练），**非**等算力 iso-FLOP。T2（F1/F2）严格判决须等算力多尺度曲线（Phase 1b，H100）。"约 1/10 参数胜出"是消融预算下的下界表述。
- **C2（小数据与短训练）**：消融为 10 万样本、1 轮、无 LR 预热；缩放律家族训练更充分（约 200 token/参数）。可引用的是相对排序，绝对困惑度为设置特定值。
- **C3（单一语言）**：全部为中文语料 + BERT-中文分词器；跨语言泛化未测。
- **C4（ET 对称项本质非因果）**：ET 的对称第二项要求 key 位置依赖未来 query 位置（因果回归测试实测泄漏约 0.11），与自回归因果性数学不可兼容。因果 LM 下该项被舍弃（RESOLVED_not_realizable）。这不是对 Hoover 等（2023）原始非因果 ET 的反驳，而是"ET 不适用于因果 LM"这一设置层结论。
- **C5（临界指数与能耗工具链于 v2.3 修正）**：临界指数修复三处缺陷——(a) 噪声 OFF/ON 在 eval 下产生逐位相同输出（模型推理不注入噪声，该对照失效），改用 shuffle 替代对照；(b) η 逐序列协方差病态饱和至 ≈1，改为全局 5 万 token 协方差；(c) Hurst 因错误的差分加补偿被压至 ≈0.15，改为标准 DFA-2（以 surrogate H=0.519 验证标定），β 因逐序列周期图拟合 $R^2$=0.14，改为 Bartlett 平均 PSD（$R^2$=0.94）。能耗修复了各家族空闲功耗相差约 87 W 的伪差异（改用全局稳健 idle 基线）。**本节 F3/F4/F5/F6 判定与能耗数字均使用修正后工具链。** 雪崩检测器的 `xmin` 异常偏大（约 5857，仅余约 1300 尾部事件）提示其阈值/定义在本尺度可能不适配，F5 的 FAIL 应附此测量有效性 caveat。
- **C6（η 度量无区分力）**：$\eta=(\lambda_{\max}-\lambda_{\min})/(\lambda_{\max}+\lambda_{\min})$ 对重尾协方差谱饱和，CID（0.997）与 Transformer（0.998）几乎相同。F6 因此判 INCONCLUSIVE 而非 PASS；建议改用参与比或谱熵重构该度量后再作判决性使用。
- **C7（记忆核长度为期中工程改动）**：v2.3 将 memory_length 由 64 提升至 512（属事后工程调整）。改动前（L=64，`cid_full` 10M 困惑度 23.62）与改动后（L=512，21.95）结果均报告，不作静默替换。
- **C8（旋度小消融效应非相关检验）**：旋度的理论角色是非平衡之必要性（命题 C3.3），应由临界指数而非消融损失检验；其小消融效应不应被引用为"旋度不重要"。
- **C9（能耗使用随机输入 token）**：解码基准使用随机 token（已在结果中标注）；每 token 能耗对输入分布基本不敏感，但建议补充真实语料复测。
- **C10（OU>FFT 幅度随规模变化）**：FFT 在 10M 的灾难性劣化（约 157）在 100M 收窄至温和（约 12.67）。OU 优于 FFT 的方向在各尺度成立，但"约 7 倍"是 10M 特定幅度，须分尺度报告。
...
作者感谢赵星宇（合肥综合性国家科学中心能源研究院中子技术应用研究中心）对本文 Phase 1 实验的独立复现，及其对配套仓库若干工程问题的反馈。
