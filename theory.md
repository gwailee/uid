...
本节给出本部分的核心命题及其完整推导。须特别说明：细致平衡破缺无关——一个非马尔可夫但满足细致平衡的过程，其过去与未来在给定现在条件下完全可以不独立，从而 I_pred > 0。这一逻辑漏洞会架空命题的核心。本文证明：**不再依赖马尔可夫假设**，改为直接建立预测信息 I_pred 与稳态熵产生率（等价地，概率环流）之间的定量下界关系。
