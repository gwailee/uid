...
**可证伪预言与初步实证**：本文据此提出**约 5 至 10 倍参数效率**的可证伪工程目标，并给出三组**已被生物大脑独立实证**的临界普适类预言：雪崩规模指数 τ ≈ 1.5（[Beggs & Plenz，2003](https://doi.org/10.1523/JNEUROSCI.23-35-11167.2003)）、Hurst 指数 H ≈ 0.7（[Linkenkaer-Hansen 等，2001](https://doi.org/10.1523/JNEUROSCI.21-04-01370.2001)）、1/f 噪声谱斜率 β ≈ 1（[He，2014](https://doi.org/10.1016/j.tics.2014.04.003)）。**本版本（v2.3）完成 Phase 1 多尺度实证**（10M/30M/100M 三尺度、中文 MiniMind 语料、3 个随机种子；含 11 路消融的三尺度扫描、临界指数与解码能耗，以及一份第三方独立复现；完整报告与全部原始数据见配套仓库）。v2.3 相对早期版本的两项关键更新：**确认 §8.5 借用的 ET 对称项在因果语言模型中本质非因果、不可实现**（见下文关于 ET 的说明）；并**完成 10M/30M/100M 三尺度消融扫描**。

**（一）框架优势（T1）获得强支持，且随规模单调放大。** 在与标准注意力骨架完全相同的前提下仅装入 UID 的三个**因果安全**物理项（旋度、色阻尼记忆核、OU 色噪声）。在 10M/30M/100M 三尺度消融（3 种子，同预算）中，`cid_full` 相对纯 Transformer 的困惑度比**随规模单调放大**：10M 为 73.58/23.62 = **3.12 倍**，30M 为 46.36/12.49 = **3.71 倍**，100M 为 41.76/10.21 = **4.09 倍**。尤为关键的是：**`cid_full` 在 10M（约 4.8M 非嵌入参数，困惑度 23.62）已显著优于纯 Transformer 在 100M（约 49M 参数，困惑度 41.76）——即 CID 以约 1/10 的参数即胜出**，方向性地支持 5–10 倍参数效率（T2）的目标。作为对照，充分训练（约 200 token/参数）的 10M 单点为 `cid_full` 7.90 vs Transformer 31.12（3.94 倍），且 CID 训练损失（2.355）亦远低于 Transformer（3.345），排除"CID 仅靠正则化取胜"的解释。色阻尼记忆核是单项贡献最大的物理项（各尺度移除均使困惑度上升 15–22%），且物理 OU 噪声优于 FFT 谱整形（幅度随规模变化：10M 约 7 倍、100M 约 1.24 倍），与第一部分第 14.2 节一致。
...
- 本节多尺度消融给出的优势比值（10M 3.12 倍、30M 3.71 倍、100M 4.09 倍）均为**消融预算（单轮训练）下的等参数困惑度比**，是参数效率的**方向性下界**，**不是**严格的参数效率测量；理论的 5–10 倍参数效率承诺（T2）须由多尺度**等算力**标度曲线检验，列为完整 Phase 1（1b，H100）待办。
- 能耗结果为**等参数（iso-parameter）开销**，**不是** C13 的等性能（iso-PPL）能效判决；后者在单一尺度下无法成立，亦列为 Phase 1b 待办。

我们以"无选择性报告、无事后调整、负面结果同等篇幅呈现"的承诺报告全部结果，包括两项 FAIL、一项无区分力的 INCONCLUSIVE，以及对 ET 项的结构性否证。v2.3 相对早期版本的两项关键更新（ET 对称项因非因果而舍弃、临界指数与能耗工具链修正）均如实记录，改动前后结果一并保留、不作替换（见 C16.8）。

#### C16.1 框架优势（T1）：随规模单调放大，CID 以约 1/10 参数胜出

在与标准注意力骨架完全相同的前提下，仅装入 UID 的三个**因果安全**物理项（旋度 $v(\varphi)$、色阻尼记忆核 $\int\gamma$、OU 色噪声 $\xi$）。10M/30M/100M 三尺度消融（1 epoch，3 种子，同预算，memory_length=64）结果如下：

| 尺度 | `cid_full` 非嵌入参数 | `cid_full` 困惑度 | `transformer` 困惑度 | 困惑度比 |
|---|---|---|---|---|
| 10M | 约 4.8M | **23.62**（23.76/23.50/23.59）| 73.58 | **3.12 倍** |
| 30M | 约 22.7M | **12.49**（12.51/12.48/12.47）| 46.36 | **3.71 倍** |
| 100M | 约 49.3M | **10.21**（10.26/10.21/10.17）| 41.76 | **4.09 倍** |

三点值得强调：

1. **优势随规模单调放大**（3.12 → 3.71 → 4.09 倍）。架构优势"越大越强"，与"基线经更充分训练/更大规模即可追平"这一典型失效模式**相反**。
2. **CID 以约 1/10 的参数胜出**：`cid_full` 在 10M（约 4.8M 非嵌入参数、困惑度 23.62）已显著优于纯 `transformer` 在 100M（约 49M 参数、困惑度 41.76）。这是参数效率的一个**强下界**信号，方向上与 5–10 倍参数效率（T2）目标一致。
3. **跨种子近零方差**，表明这是稳定的架构性差异，而非幸运种子。

作为**充分训练对照**，三家族缩放律训练（约 200 token/参数，10M，3 种子）给出：`transformer` 31.12、`transformer_plus_tricks` 31.23、`cid_full` **7.90**（7.88/7.90/7.90，标准差 ≈ 0.01），即充分训练下 10M 单点比值为 **3.94 倍**；CID 训练损失（2.355）亦远低于 Transformer（3.345），排除"CID 仅靠正则化取胜"。

（口径提示：三尺度消融与缩放律对照均为 memory_length=64；两者差异来自训练预算不同（消融 1 epoch vs 缩放律约 200 token/参数），不可直接连成一条标度曲线；严格 T2 判决须等算力多尺度曲线，见 C16.8 caveat C1。）
...
- **能耗**（见 C16.5）：`transformer_plus_tricks` 消耗最高的每 token 能耗，换来最差的质量。
...
| 旋度（$v(\varphi)$） | +0.2 ~ +0.4% | 消融效应小；其*必要性*应由临界指数检验（命题 C3.3），见 C16.4 |
| 色噪声（存在性） | ≈ 0% | 存在性影响小；关键在噪声*类型*（OU vs FFT）而非存在 |
| OU → FFT 噪声 | 约 7 倍（10M）/ 约 1.24 倍（100M）更差 | OU 优于 FFT，支持 §14.2；但优势幅度随规模收窄 |

两点须诚实标注：

- **记忆核为主导物理项**，其相对贡献在三个尺度稳定在 15–22%，直接对应理论所述 Transformer 砍掉的 $\int\gamma$ 色阻尼项。
...
**本节最重要的诚实结论**：四个临界指数**均无法把 CID 与普通 Transformer 区分开**（H 0.80 vs 0.81；β 0.57 vs 0.71；η 0.997 vs 0.998）。这恰好印证本文摘要早已申明的立场——这三组普适指数预言区间相当宽、可证伪强度有限，能排除白噪声等平凡情形，却难以把 CID 与其他同样表现出自组织临界的模型区分开。因此本文**不将临界指数作为 CID 优于 Transformer 的区分性证据**，仅作为"CID 表现出与生物大脑一致的临界统计特征"的描述性旁证；真正具有区分力的判决点是参数效率承诺与关联长度标度（见 C16.6）。
...
|---|---|---|---|
| **T1**：CID 物理项 > Transformer | 是 | **支持，且随规模放大**（3.12→3.71→4.09 倍；约 1/10 参数胜出）| 多尺度消融（C16.1）+ 缩放律 |
| **T2**：5–10× 参数效率 | 是 | 方向性强支持（消融预算下下界约 10 倍参数）；判决待定 | 等算力多尺度曲线（F1/F2），Phase 1b |
| **C3.3 / 旋度必要性**（非平衡） | 是 | 部分：Hurst 通过；β/雪崩未达；无基线区分 | 临界指数（F3–F6） |
| **§14.2**：OU > FFT 色噪声 | 是 | **各尺度支持**（幅度随规模变化：10M~7×、100M~1.24×）| 消融（C16.3） |
| **色阻尼记忆核（$\int\gamma$）** | 是 | **支持，主导项**（各尺度移除 +15~22%）| 消融（C16.3） |
...
- **C7（旋度小消融效应非相关检验）**：旋度的理论角色是非平衡之必要性（命题 C3.3），应由临界指数而非消融损失检验；其小消融效应不应被引用为"旋度不重要"。
- **C8（能耗使用随机输入 token）**：解码基准使用随机 token（已在结果中标注）；每 token 能耗对输入分布基本不敏感，但建议补充真实语料复测。
- **C9（OU>FFT 幅度随规模变化）**：FFT 在 10M 的灾难性劣化（约 157）在 100M 收窄至温和（约 12.67）。OU 优于 FFT 的方向在各尺度成立，但"约 7 倍"是 10M 特定幅度，须分尺度报告。
...
> **说明**：复现使用 commit `53f2aa06`（早于 v2.3 的 ET 修正），其临界指数工具为修复前版本，故复现的 Hurst（0.77）与本文修正后值（0.803）略有差异，但方向一致（均落入 [0.6,0.8] 且无法区分基线）。

须诚实指出：该复现进一步确证了本文的**参数效率方向结论稳健（单轮 3.12× → 充分训练 3.94×，随训练放大）**，同时也独立复现了本文**尚未成立的判决性预言**——iso-PPL 能效（C13）与临界涌现在 10M 单点上均不可判定/未获支持。这与本文 C16.8 列出的边界完全一致。
