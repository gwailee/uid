...
**可证伪预言与初步实证**：本文据此提出**约 5 至 10 倍参数效率**的可证伪工程目标，并给出三组**已被生物大脑独立实证**的临界普适类预言：雪崩规模指数 τ ≈ 1.5（[Beggs & Plenz，2003](https://doi.org/10.1523/JNEUROSCI.23-35-11167.2003)）、Hurst 指数 H ≈ 0.7（[Linkenkaer-Hansen 等，2001](https://doi.org/10.1523/JNEUROSCI.21-04-01370.2001)）、1/f 噪声谱斜率 β ≈ 1（[He，2014](https://doi.org/10.1016/j.tics.2014.04.003)）。**本版本（v2.2）完成 Phase 1 四类实验**（10M 规模，中文 MiniMind 语料，3 个随机种子；含 11 路消融、三家族缩放律训练、临界指数与解码能耗；完整报告与全部原始数据见配套仓库）。

**（一）框架优势（T1）获得强支持，且随训练放大。** 在与标准注意力骨架完全相同的前提下仅装入 UID 的三个物理项（旋度、色阻尼记忆核、OU 色噪声）：在单轮消融中即把困惑度从纯 Transformer 基线的 73.58 降至 22.87（**3.22 倍**，z = 182）；在充分训练（约 200 token/参数）的缩放律设置中，`cid_full` 困惑度降至 **7.90**（三种子 7.88/7.90/7.90，标准差 ≈ 0.01）而 Transformer 为 31.12——即 **3.94 倍优势，且 CID 的非嵌入参数更少（4.83M vs 5.12M）**。该优势相对消融阶段不降反升，与"基线经更充分训练即可追平"这一典型失效模式相反。色阻尼记忆核是单项贡献最大的物理项（移除使困惑度上升 21%），且物理 OU 噪声显著优于 FFT 谱整形（6.9 倍，z = 37），与第一部分第 14.2 节一致。

**（二）"注意力并不够"在三组独立实验中复现。** 已知 Transformer 技巧（深度卷积、线性对易子、噪声注入）在消融中增益 < 1%；在缩放律中 `transformer_plus_tricks`（31.23）甚至略差于纯 Transformer（31.12），却多耗约 2.3 倍算力。CID 的优势因此不能归因于"堆叠更多技巧"。

**（三）临界普适类预言获部分支持，且如理论所预期无法区分基线。** 在修正测量工具后（以 shuffle 替代对照 Hurst = 0.519 ≈ 0.5 验证估计器无偏，谱拟合 R² = 0.94）：Hurst 指数 H = 0.803 落入预言区间 [0.6, 0.8] 且远高于替代对照（**F4 通过**），表明隐状态沿 token 轴存在真实长程正相关；而 1/f 谱斜率 β = 0.572 略低于 [0.7, 1.3]（**F3 未达**），雪崩尾部未通过幂律检验（**F5 未达**，附测量有效性说明），各向异性 η = 0.997 虽 > 0.5 但与 Transformer 基线（0.998）几乎相同而**无区分力**（**F6 不确定**）。**关键的诚实结论是：四个临界指数均无法把 CID 与普通 Transformer 区分开**——这恰好印证本文摘要早已申明的立场：这些普适指数区间相当宽、可证伪强度有限，能排除白噪声等平凡情形，却难以把 CID 与其他同样表现出自组织临界的模型区分开；它们仅作为"CID 表现出与生物大脑一致的临界统计特征"的描述性旁证，**而非** CID 优于 Transformer 的独立证据。

**（四）能耗在修正测量后可干净测得，但其判决性检验须俟多尺度。** 在稳健的全局空闲基线下（61.9 W，窗口间离散 0.06 W；修复了此前各家族空闲功耗相差约 87 W 的测量伪差异），等参数解码下 `cid_full` 的每 token 超空闲能耗（0.160 mJ）仅比 Transformer（0.141 mJ）高约 **13%**，且低于 `transformer_plus_tricks`（0.164 mJ），同时参数最少、峰值功率最低——结合 3.9 倍困惑度优势，即"以约 13% 的能耗代价换取约 3.9 倍质量"。
**须诚实标注全部边界**：上述 3.94 倍是**单一尺度（10M）下的等参数损失比**，13% 是**等参数能耗开销**，二者均与 5–10 倍参数效率（T2）及 ≥3 倍能效（C13）的目标方向一致，但**都不直接检验之**；T2 须多尺度等算力标度曲线、C13 须等性能（iso-PPL）能耗曲线，两者在单一尺度下均无法成立（10M Transformer 无法达到 CID 的困惑度，故等性能比当前仅为下界），列为完整 Phase 1（1b）的判决性待办。此外，借用自 [Hoover 等（2023）](https://arxiv.org/abs/2302.07253)的 ET 对称项在本因果语言模型设置下**未带来收益**（预注册条件 F8 判为 FAIL），但这一否证只针对**借用的非首创组件**，且因 UID 的优势在移除 ET 后不降反升，反而"提纯"了"优势源于 UID 自身物理项"这一归属（详见第一部分第 16 章的初步实证小节）。综上，**真正具有区分力的判决点是参数效率承诺与关联长度标度**（临界指数本身区分力不足这一点，已被本轮实测进一步证实）。UID 的参数效率预言与 Alman-Song-Gupta 复杂度下界**互补而非冲突**——前者通过脱离 softmax-attention 接口、进入不同复杂度类而获得收益。
...
### C第16章 Phase 1 初步实证：四类实验结果（v2.2）

本节报告 UID v2.2 随附的 Phase 1 初步实证。实验在中文 MiniMind 预训练语料（10 万条子集，约 1000 万 token）上、10M 参数规模、3 个随机种子（[42, 43, 44]）、单张 NVIDIA RTX 4090 上完成，涵盖四类实验：**(1) 11 路组件消融、(2) 三家族缩放律家族训练、(3) 临界指数测量、(4) 解码能耗测量**。完整报告、逐种子原始记录、提交哈希与数据 SHA-256 见配套仓库 [`results/phase1/REPORT.md`](https://github.com/gwailee/uid)。

须在开篇即明确本节结论的**双重边界**，以免任何过度解读：

- 本节给出的所有优势比值（消融 3.22 倍、缩放律 3.94 倍）均为**单一尺度（10M）下的等参数损失/困惑度比**，**不是参数效率测量**；理论的 5–10 倍参数效率承诺（T2）须由多尺度等算力标度曲线检验，列为完整 Phase 1（1b）待办。
- 能耗结果为**等参数（iso-parameter）开销**，**不是** C13 的等性能（iso-PPL）能效判决；后者在单一尺度下无法成立，亦列为 Phase 1b 待办。

我们以"无选择性报告、无事后调整、负面结果同等篇幅呈现"的承诺报告全部结果，包括三项 FAIL 与一项无区分力的 INCONCLUSIVE。

#### C16.1 框架优势（T1）：消融与缩放律双重证据

**消融（单轮训练）。** 在与标准注意力骨架完全相同的前提下，仅装入 UID 的三个物理项（旋度 $v(\varphi)$、色阻尼记忆核 $\int\gamma$、OU 色噪声 $\xi$），即可把困惑度从纯 Transformer 基线的 73.58 降至 22.87（`cid_full_no_et`），即 **3.22 倍优势**。11 路变体呈现清晰的两个梯队：所有 CID 变体聚集于困惑度 22.9–28.7，所有 Transformer 变体聚集于 72.8–73.6。

**缩放律（充分训练，约 200 token/参数）。** 三个家族各训练 3 个种子至充分收敛，结果如下：

| 家族 | 非嵌入参数 | 困惑度（种子 42/43/44） | 均值 | 训练损失（均值） |
|---|---|---|---|---|
...
充分训练后，`cid_full` 困惑度降至 **7.90**（标准差 ≈ 0.01），相对 Transformer（31.12）达 **3.94 倍优势，且 CID 的非嵌入参数更少（4.83M vs 5.12M）**。三点值得强调：

1. **优势随训练放大**（消融 3.22 倍 → 缩放律 3.94 倍），与"基线经更充分训练即可追平"这一架构优势的典型失效模式**相反**。
2. **训练损失同样大幅领先**（2.355 vs 3.345），排除了"CID 仅靠某种正则化或泛化技巧取胜"的解释——CID 在训练分布上即学得更好，泛化差距更大。
3. **跨种子近零方差**，表明这是稳定的架构性差异，而非幸运种子。

#### C16.2 "注意力并不够"：三组独立复现

已知 Transformer 技巧在三组独立实验中均无效：

- **消融**：五个 Transformer 变体（baseline、+conv、+linear、+noise、+all\_tricks）困惑度差异 < 1%（标准差 < 0.01）；深度卷积、线性对易子、噪声注入，无论单独或组合，均无可测增益。
- **缩放律**：`transformer_plus_tricks`（31.23）甚至略差于纯 `transformer`（31.12），却多耗约 **2.3 倍**算力（wall-clock）。
...
`run_ablation.py` 发射三组预注册临界对照：

| 对照 | 含义（理论章节） | `cid_full` 损失 | 对照组损失 | 优势 | z-score | 判定 |
|---|---|---|---|---|---|---|
| **A** | CID 物理框架 vs 已知技巧（T1 核心论断） | 3.162 | `transformer_plus_all_tricks` 4.295 | **+1.133** | 182.19 | ✅ supported |
| **C** | §14.2 OU vs FFT 噪声 | 3.162 | `cid_full_fft_noise` 5.131 | **+1.969** | 37.14 | ✅ supported |
| **B** | §8.5 ET 对称项（F8） | 3.162 | `cid_full_no_et` 3.130 | **−0.032** | −6.39 | ❌ not_supported |

各物理项的单项贡献（相对 `cid_full`，困惑度 23.62）：

| 切除的组件 | 变体 | 困惑度 | Δ vs `cid_full` | 解读 |
|---|---|---|---|---|
| 色阻尼记忆核（ $\int\gamma$ ） | `cid_no_memory` | 28.65 | **+21.3%** | **单项贡献最大的物理项**；对应 Transformer 砍掉的色阻尼项 |
| 色噪声（存在性） | `cid_no_noise` | 23.79 | +0.7% | 存在性影响小；关键在噪声*类型*（见对照 C） |
| 旋度（ $v(\varphi)$ ） | `cid_no_vortex` | 23.71 | +0.4% | 消融效应小；其*必要性*应由临界指数检验（命题 C3.3），见 16.4 |
| 禁用 ET 对称项 | `cid_full_no_et` | 22.87 | **−3.2%** | ET（借自 Hoover 2023）无收益；F8 FAIL |
| OU → FFT 噪声 | `cid_full_fft_noise` | 169.93 | **+619%** | OU 远优于 FFT；支持 §14.2 |

**色阻尼记忆核是单项贡献最大的物理项**（移除使困惑度上升 21.3%），直接对应理论所述 Transformer 砍掉的 $\int\gamma$ 色阻尼项。**OU 色噪声远优于 FFT 谱整形**（`cid_full_fft_noise` 困惑度 169.93，为全套件最差，且方差最大、训练最不稳定），是 §14.2 选择 OU 为物理默认的最强单点证据。旋度与色噪声的单项消融效应虽小，但分别对应"必要性应由临界指数检验"（旋度）与"关键在类型而非存在"（噪声），见下。

#### C16.4 临界指数：部分支持，且无法区分基线

临界指数测量在训练好的 `cid_full_10M` 与 `transformer_10M` 检查点上、噪声注入关闭、2000 序列 × 512 token 上完成。**须说明：本轮测量在工具链层面修正了三处缺陷**（见 caveat C5），修正后以 **shuffle 替代对照的 Hurst = 0.519 ≈ 0.5**（白噪声值）与**谱拟合 $R^2 = 0.94$** 验证估计器已无偏。修正后结果：
...
- **F4（Hurst）通过**：H = 0.803 落入/紧贴预言区间，且**远高于 shuffle 替代对照（0.519）**，表明隐状态沿 token 轴存在真实长程正相关，与人类皮层观测值（[Linkenkaer-Hansen 等，2001](https://doi.org/10.1523/JNEUROSCI.21-04-01370.2001)）方向一致。
...
- **F6（η）不确定**：η = 0.997 > 0.5，但与 Transformer 基线（0.998）几乎相同——该度量 $\eta=(\lambda_{\max}-\lambda_{\min})/(\lambda_{\max}+\lambda_{\min})$ 对重尾协方差谱饱和趋于 1，**无区分力**，故判 INCONCLUSIVE 而非 PASS（度量重构建议见 caveat C6）。
...
能耗在解码模式（每次生成 64 token，batch 8 × seq\_len 256）下、以 pynvml @ 25 Hz 高频采样测量。**须说明：本轮采用稳健的全局空闲基线**（3 窗口中位数 = 61.9 W，窗口间离散仅 0.06 W），修复了此前各家族空闲功耗相差约 87 W（CID 124 W vs Transformer 211 W）导致 above-idle 列不可比的测量伪差异（见 caveat C5）。
...
等参数下，`cid_full` 每 token 超空闲能耗仅比 Transformer 高约 **13%**，且**低于** `transformer_plus_tricks`（后者以最高能耗换最差质量），同时 CID 参数最少、峰值功率最低（259.9 W vs Transformer 275.1 W）——其开销主要来自串行延迟而非峰值功率，对工程优化更友好。结合 16.1 的 3.94 倍困惑度优势，可概括为：**CID 以约 13% 的每 token 能耗代价，换取约 3.9 倍的困惑度优势**。

须再次强调：上述为**等参数开销**，**不是** C13 的"等性能下 ≥3 倍能效"判决（预注册条件 F7）。C13 须在等性能（iso-PPL）下测量——即"达到 CID 困惑度 7.90 所需的 Transformer 耗多少能"，而 10M 尺度的 Transformer 根本无法达到该困惑度（停在 31.12），故 iso-PPL 能耗比当前未定义（测量工具正确地拒绝外推）。F7 须俟多尺度曲线，列为 Phase 1b 待办。
...
| ❌ FAIL（3） | F3（β）、F5（雪崩）、F8（借来的 ET） | β=0.572 略低；雪崩非幂律；ET 无收益（非首创组件） |
| ⚠️ INCONCLUSIVE（1） | F6（η） | η>0.5 但与基线无区分，度量饱和 |
| ⏳ ABSTAIN（3） | F1、F2（多尺度未跑）、F7（单尺度 iso-PPL 无法测） | T2 与 C13 的**唯一判决点**，列入 Phase 1b |

须特别指出：F3/F5 的 FAIL 与 F6 的无区分力，均涉及**B 级（理论严格、待实证）的临界普适类预言**，本文已申明这些预言区分力有限、非核心判决点；而 F8 的 FAIL 仅针对**借自 Hoover 等（2023）的非首创 ET 组件**，且因 UID 优势在移除 ET 后不降反升（22.87 < 23.62），反而"提纯"了"优势源于 UID 自身物理项"这一归属。**核心判决点 T2（参数效率）与 C13（能效）尚未被检验**——本节全部正面结果（T1、Hurst、OU>FFT）与全部负面结果（β、雪崩、η、ET）均不改变这一待办状态。

#### C16.7 与理论命题的对应

| 理论命题 | 是否 UID 原创 | 本轮判定 | 正确检验手段 |
|---|---|---|---|
| **T1**：CID 物理项 > Transformer | 是 | **支持**（消融 3.22×、z=182；缩放律 3.94×、随训练放大、参数更少） | 消融 A + 缩放律 |
| **T2**：5–10× 参数效率 | 是 | 未检验（方向一致） | 多尺度标度曲线（F1/F2） |
| **C3.3 / 旋度必要性**（非平衡） | 是 | 部分：Hurst 通过；β/雪崩未达；无基线区分 | 临界指数（F3–F6） |
| **§14.2**：OU > FFT 色噪声 | 是 | **支持**（6.9×，z=37） | 消融 C |
| **色阻尼记忆核（ $\int\gamma$ ）** | 是 | **支持**（主导项，移除 +21%） | 消融 |
| **C13**：iso-PPL 下 ≥3× 能效（F7） | 是 | 单尺度无法检验（iso-param 开销 1.13×） | 多尺度 iso-PPL（F7） |
| **ET 对称项（§8.5）** | **否**（Hoover 2023） | **此处证伪**（−3.2%，z=−6.39）；因果离散化 caveat C4 | 消融 B |

### C16.8 局限与逐条注意事项（caveat）

引用本节任何结果时，须同时引用以下逐条注意事项：

- **C1（单一尺度）**：全部实验在 10M 单尺度完成。3.94 倍损失比与 13% 能耗开销均与 T2（5–10× 参数效率）、C13（≥3× 能效）方向一致，但**不直接检验之**；F1/F2/F7 须多尺度曲线，为 Phase 1b 判决性待办。
- **C2（小数据与短训练）**：消融为 10 万样本、1 轮、无 LR 预热；缩放律家族训练更充分（约 200 token/参数）。可引用的是相对排序，绝对困惑度为设置特定值。
- **C3（单一语言）**：全部为中文语料 + BERT-中文分词器；跨语言泛化未测。
- **C4（F8 仅检验一种因果离散化的 ET）**：本代码库的 ET 项在自回归（因果）掩码下评估；Hoover 等的双 softmax ET 为非因果设置。F8 的 FAIL 应读作"本代码库的因果 ET 实现在此无收益"，**而非**对原始（非因果）Energy Transformer 理论的反驳。
- **C5（临界指数与能耗工具链曾于本期修正）**：临界指数修复三处缺陷——(a) 噪声 OFF/ON 在 eval 下产生逐位相同输出（模型推理不注入噪声，该对照失效），改用 shuffle 替代对照；(b) η 逐序列协方差病态饱和至 ≈1，改为全局 5 万 token 协方差；(c) Hurst 因错误的差分加补偿被压至 ≈0.15，改为标准 DFA-2（以 surrogate H=0.519 验证标定），β 因逐序列周期图拟合 $R^2=0.14$，改为 Bartlett 平均 PSD（ $R^2=0.94$ ）。**本节 F3/F4/F5/F6 判定均使用修正后工具链。** 雪崩检测器的 `xmin` 异常偏大（约 5857，仅余约 1300 尾部事件）提示其阈值/定义在本尺度可能不适配，F5 的 FAIL 应附此测量有效性 caveat。
...
- **C7（对照 A / 缩放律可能存在训练预算不对称）**：Transformer 家族与 CID 采用同一调度（未调 LR 预热）。建议补充公平性检验（Transformer 家族加预热 + 更长调度）；但驱动 F8 的组内对照 `cid_full_no_et` vs `cid_full` 使用同一预算，不受此 caveat 影响；缩放律中优势随训练*放大*而非缩小，亦部分缓解此疑虑。
- **C8（旋度小消融效应非相关检验）**：旋度的理论角色是非平衡之必要性（命题 C3.3），应由临界指数而非消融损失检验；+0.4% 不应被引用为"旋度不重要"。
