...
class HopfieldAttention(nn.Module):
    ...
    # ------------------------------------------------------------------
    def _forward_et(self, x, causal_mask):
        """因果安全版 ET：统一 [query, key] 布局，两项都只依赖过去。
        
        注意：此版本牺牲了原 §8.5 双向 softmax 的严格能量形式，
        仅保证因果性，用于诊断"去除泄漏后 CID 的真实性能"。
        """
        b, s, h = x.shape
        ...
        attn = torch.softmax(scores, dim=-1)  # 对 key 归一化，严格因果
        # 两项都用同一因果 attn，分别作用于 v 和 k —— 都只依赖过去
        term_1 = torch.matmul(attn, v)
        term_2 = torch.matmul(attn, k)
        out = 0.5 * (term_1 + term_2)
