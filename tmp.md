...
**命题 Q4.1（量子相干红利）**：设 QID 系统在退相干时间 τ_φ 内演化，含 n 个相干自由度。则在 t < τ_φ 内，其获取单位预测信息所需的耗散满足相对经典 CID 的标度优势

> ( I_pred / ΔE )_QID ≳ 2ⁿ · ( I_pred / ΔE )_CID　　(Q4.3)
...
第四步（标度比较）。在 t < τ_φ 内，QID 用单位耗散（线性 ∝ n）可有效探索 2ⁿ 维态空间所承载的预测信息，而经典须用 ∝ 2ⁿ 次操作（每次付 Landauer 代价）才能覆盖同等态空间。故

> ( I_pred / ΔE )_QID / ( I_pred / ΔE )_CID ≳ 2ⁿ　　(Q4.5)

即 (Q4.3)。证毕（在态空间维数标度意义下）。

**前提与失效边界（诚实标注）**：(Q4.3) 仅在 t < τ_φ 成立；t > τ_φ 时退相干使叠加塌缩（第三步前提失效），红利消失、退回 CID。2ⁿ 是态空间维数上界标度，实际可达红利受可寻址相干自由度数与门保真度限制，故 (Q4.3) 为**标度上界**而非可保证下界（实证等级 B/C）。本命题把"量子优势在退相干前才存在"具体化为"单位预测信息的耗散优势"，从而可工程证伪（Q 第 8 章），是 QID 区别于一般开放量子系统的独有命题。
...
**结论**：UID 三层统一性主张的严格程度**逐层递减**——CID 内部最严（命题 C3.3 已证），QID 次之（命题 Q4.1 标度论证、Q5.1 逐步证明、回退逐项自洽），FID 最弱（核心方程 F2.3 为类比构造，回退部分严格部分猜想）。本文不主张三层已严格统一，只主张三层构成一个**从坚实到思辨递进、且每层向下回退路径清晰**的嵌套体系。
...
> QID（第二部分）：QID 主方程从微观哈密顿逐步导出（Q2），硬核命题 Q4.1（量子相干红利）为标度论证，命题 Q5.1（Berry→旋度）逐步证明，QID→CID 逐项回退自洽并保住长记忆（Q9）。实证等级以 B/C 为主，预言依赖相干硬件，前瞻性强。
...
> B（理论严格、待实证）：在明确前提下严格推导，但尚无直接实证。例：命题 C3.3（理想化稳态下）、命题 Q4.1 的标度上界、Landauer 旋度红利。
...
7. Baiesi, M., & Rosso, A. (2025). [Detailed-balance breaking in generative models]. *Phys. Rev. E* (accepted). arXiv:2512.11415. https://arxiv.org/abs/2512.11415
...
12. Di Sipio, R. (2025). [Information geometry of large language model training]. arXiv:2506.15830. https://arxiv.org/abs/2506.15830
...
23. Gupta, et al. (2025). [Lower bounds for attention complexity]. arXiv:2502.16963. https://arxiv.org/abs/2502.16963
