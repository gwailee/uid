...
$\hbar$ 为约化普朗克常数；  
$k_B T$ 为热浴温度能标；  
$\hat{\varphi}$、$\hat{p}$ 为系统慢变量算符及其共轭动量；  
$\{\hat{x}_k, \hat{p}_k\}$ 为第 $k$ 个浴振子的坐标与动量算符，频率 $\omega_k$、质量 $m_k$；  
$c_k$ 为系统与第 $k$ 个浴振子的耦合强度；  
$J(\omega)$ 为系统-浴耦合谱密度，沿用第一部分记号（式 C5.1），取亚欧姆形式 $J(\omega) \propto \omega^s$，$0 < s < 1$；  
$\hat{\rho}(t)$ 为系统约化密度矩阵；  
$\tau_\varphi$ 为退相干时间；  
$\tau_B$ 为浴关联时间；  
$\tau_S$ 为系统本征演化时标。
