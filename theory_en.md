...
**Falsifiable predictions and preliminary empirical results**: On this basis the paper proposes a falsifiable engineering target of **~5–10× parameter efficiency**, together with three critical-universality-class predictions that have been **independently confirmed in the biological brain**: avalanche-size exponent τ ≈ 1.5 ([Beggs & Plenz, 2003](https://doi.org/10.1523/JNEUROSCI.23-35-11167.2003)), Hurst exponent H ≈ 0.7 ([Linkenkaer-Hansen et al., 2001](https://doi.org/10.1523/JNEUROSCI.21-04-01370.2001)), and 1/f noise-spectrum slope β ≈ 1 ([He, 2014](https://doi.org/10.1016/j.tics.2014.04.003)). **This version (v2.3) completes a multi-scale Phase 1 empirical study** (three scales 10M/30M/100M, Chinese MiniMind corpus, 3 random seeds; comprising a three-scale sweep of the 11-way ablation, critical exponents, and decode energy, plus an independent third-party reproduction; the full report and all raw data are in the companion repository). Relative to earlier versions, v2.3 makes three key updates: the memory-kernel length is raised from 64 to 512; three toolchain measurement bugs are corrected (see (iii) below); and the borrowed §8.5 ET symmetric term is **shown to be intrinsically non-causal and therefore not realizable in a causal language model** (see the ET discussion below).

**(i) The framework advantage (T1) is strongly supported and GROWS monotonically with scale.** With an attention backbone identical to the standard one, adding only UID's three **causal-safe** physical terms (vortex, colored-damping memory kernel, OU colored noise), the `cid_full`/`transformer` perplexity ratio in the 10M/30M/100M multi-scale ablation (memory_length=512, 3 seeds) **grows monotonically with model size**: 73.58/21.95 = **3.35×** at 10M, 46.36/12.49 = **3.71×** at 30M, and 41.76/10.21 = **4.09×** at 100M. Most importantly, **`cid_full` at 10M (~4.8M non-embedding params, PPL 21.95) already substantially outperforms plain `transformer` at 100M (~49M params, PPL 41.76) — i.e. CID wins at roughly 1/10 the parameters**, a strong directional lower bound toward the 5–10× parameter-efficiency target (T2). For reference, the fully-trained single point (≈200 tokens/param, memory_length=64) at 10M gives `cid_full` 7.90 vs `transformer` 31.12 (3.94×). The colored-damping memory kernel is the single largest physical contributor (its removal raises perplexity by 15–22% at every scale), and physical OU noise outperforms FFT spectral shaping (magnitude scale-dependent: ~7× at 10M, ~1.24× at 100M), consistent with Part I §14.2.

**(ii) "Attention is not all you need" is consistently replicated across all three scales and the fully-trained setting.** Known Transformer tricks (depthwise conv, linear commutator, noise injection) give < 1% benefit at each of 10M/30M/100M; in the fully-trained scaling-law setting `transformer_plus_tricks` (31.23) is even slightly worse than plain Transformer (31.12) while costing ~2.3× the compute. CID's advantage therefore cannot be attributed to "stacking more tricks" and arises from architecture-level physical reconstruction.

**(iii) The critical-universality predictions receive partial support and, as the theory anticipates, do not discriminate the baseline.** After correcting the measurement tooling (v2.3 fixed three bugs; validated by a shuffle-surrogate Hurst = 0.519 ≈ 0.5 confirming the estimator is unbiased, and a spectral-fit R² = 0.94): the Hurst exponent H = 0.803 falls inside the predicted band [0.6, 0.8] and far exceeds the surrogate (**F4 PASS**), indicating genuine long-range positive correlation along the token axis; whereas the 1/f slope β = 0.572 is just below [0.7, 1.3] (**F3 not met**), the avalanche tail fails the power-law test (**F5 not met**, with a measurement-validity note), and the anisotropy η = 0.997, though > 0.5, is essentially identical to the Transformer baseline (0.998) and thus **non-discriminating** (**F6 inconclusive**). **The key honest conclusion is that none of the four critical exponents can separate CID from a plain Transformer** — precisely confirming the stance already declared in this abstract: these universal-class predictions have wide bands and limited falsifying power; they can rule out trivial cases such as white noise but cannot separate CID from other models that likewise exhibit self-organized criticality. They serve only as descriptive corroboration that "CID exhibits brain-consistent critical statistics," **not** as independent evidence that CID is superior to Transformer.

**(iv) Energy is cleanly measurable after the measurement fix, but its decisive test awaits multiple scales.** Under a robust global idle baseline (61.9 W, inter-window spread 0.06 W; fixing a prior artifact where per-family idle power differed by ~87 W), at iso-parameter decode the per-token above-idle energy of `cid_full` (0.160 mJ) is only ~**13%** higher than Transformer's (0.141 mJ) and *lower* than `transformer_plus_tricks` (0.164 mJ), while CID uses the fewest parameters and has the lowest peak power — i.e., combined with the ~3.9× perplexity advantage, "a ~3.9× quality gain for a ~13% per-token energy cost."

**All boundaries must be stated honestly**: the above multi-scale advantage ratios (3.35–4.09×) are a **directional lower bound under the ablation budget (single-epoch training)**, and the 13% is an **iso-parameter energy overhead**; both are directionally consistent with the 5–10× parameter-efficiency (T2) and ≥3× energy-efficiency (C13) targets but **do not directly test them**. T2 requires multi-scale equal-compute scaling curves and C13 requires iso-performance (iso-PPL) energy curves, neither of which can hold at a single scale (the Transformer cannot reach CID's perplexity at that scale, so the iso-performance ratio is currently only a lower bound); both are decisive to-dos for full Phase 1 (1b, on H100). In addition, regarding the ET symmetric term borrowed from [Hoover et al. (2023)](https://arxiv.org/abs/2302.07253) (pre-registered condition F8), v2.3 provides a **structural conclusion**: the symmetric second term of the negative gradient of the ET energy requires key positions to depend on future query positions, causing future-token leakage in an autoregressive causal LM (a causality regression test measures ~0.11), i.e. **ET's dual-softmax symmetry and autoregressive causality are mathematically incompatible**. The term is therefore dropped in the causal-LM setting, ET reduces to standard causal attention, and `cid_full` is numerically identical to `cid_full_no_et`. This conclusion **supersedes** the earlier "F8 judged FAIL" wording: the correct statement is not "ET is useless" but **ET is inapplicable to causal language models**; accordingly, CID's entire advantage comes from its three **causal-safe** physical terms and is independent of the borrowed ET component (see the preliminary-empirical subsection in Part I §16). In sum, **the genuinely discriminating decision points are the parameter-efficiency promise and correlation-length scaling** (the limited discriminating power of the critical exponents themselves having now been further confirmed empirically). UID's parameter-efficiency prediction is **complementary to, not in conflict with**, the Alman–Song–Gupta complexity lower bounds — the former gains its advantage by leaving the softmax-attention interface and entering a different complexity class.
...
All references in this paper provide clickable DOIs or open-access hyperlinks reaching the primary sources, and every quantitative claim is explicitly labelled with an evidence grade (A verified / B theoretically rigorous but empirically pending / C falsifiable engineering target / D philosophical conjecture). The companion code repository ([github.com/gwailee/uid](https://github.com/gwailee/uid)) provides a reference engineering implementation of CID and a falsifiable-verification suite; all core predictions are reproducible within hours on a single GPU. **This version (v2.3) ships the full report of the multi-scale Phase 1 empirical study (containing per-seed records for all 11 variants across the three scales 10M/30M/100M × 3 seeds, the commit hash, data SHA-256, negative results, and an independent third-party reproduction), following our commitment to "no selective reporting, no retroactive adjustment, negative results reported with equal prominence"; therein the FAILs of F3 (β) and F5 (avalanche), the non-discrimination of F6 (η), and F8 (the ET term being non-realizable due to non-causality) are all reported with equal prominence alongside the supported T1 trend, F4 (Hurst), and OU-over-FFT. The v2.3 engineering changes relative to earlier versions (memory kernel 64→512, ET dropped, critical-exponent tooling fixes) are all faithfully recorded, with the before/after results both retained and nothing silently replaced. We reiterate: the multi-scale ablation advantage ratios are a directional lower bound under the ablation budget, and the decisive tests of T2 and C13 must be completed on equal-compute multi-scale scaling curves and iso-performance energy curves, deferred to Phase 1b (H100). When citing this version's experimental results, one must attach the v2.3 commit hash and the per-item caveats listed in §6 of the report.**
...
- The multi-scale ablation advantage ratios (3.35× at 10M, 3.71× at 30M, 4.09× at 100M) are **iso-parameter perplexity ratios under the ablation budget (single-epoch training)** and are a **directional lower bound** on parameter efficiency, **not** a strict parameter-efficiency measurement; the theory's 5–10× parameter-efficiency promise (T2) requires multi-scale **equal-compute** scaling curves, deferred to full Phase 1 (1b, H100).
...
We report all results under our commitment to "no selective reporting, no retroactive adjustment, negative results with equal prominence," including two FAILs, one non-discriminating INCONCLUSIVE, and a structural refutation of the ET term. The three v2.3 engineering changes (memory-kernel length 64→512, dropping the non-causal ET symmetric term, and the critical-exponent tooling fixes) are all disclosed, with before/after results retained and nothing silently replaced (see C16.8).

### C16.1 Framework advantage (T1): grows monotonically with scale, CID wins at ~1/10 the parameters

With an attention backbone identical to the standard one, adding only UID's three **causal-safe** physical terms (vortex $v(\varphi)$, colored-damping memory kernel $\int\gamma$, OU colored noise $\xi$). The 10M/30M/100M multi-scale ablation (memory_length=512, 1 epoch, 3 seeds) yields:

| Scale | `cid_full` non-emb params | `cid_full` perplexity | `transformer` perplexity | Perplexity ratio |
|---|---|---|---|---|
...
1. **The advantage grows monotonically with scale** (3.35 → 3.71 → 4.09×). An architectural advantage that grows with size is the opposite of the classic failure mode where a baseline catches up given more training or a larger scale — the way many architectural improvements ultimately fail; here the reverse holds.
2. **CID wins at roughly 1/10 the parameters**: `cid_full` at 10M (~4.8M non-embedding params, PPL 21.95) already substantially outperforms plain `transformer` at 100M (~49M params, PPL 41.76). This is a **strong lower bound** on parameter efficiency, directionally consistent with the 5–10× (T2) target.
3. **Near-zero cross-seed variance** (e.g. `cid_full` std ≈ 0.05 at 100M), indicating a stable architectural difference rather than a lucky seed.

As a **fully-trained reference**, three-family scaling-law training (≈200 tokens/param, memory_length=64, 10M, 3 seeds) gives `transformer` 31.12, `transformer_plus_tricks` 31.23, and `cid_full` **7.90** (7.88/7.90/7.90, std ≈ 0.01) — a **3.94×** ratio at this fully-trained 10M point, with CID's training loss (2.355) also far below Transformer's (3.345), ruling out a "CID merely wins via regularization" explanation.

(Budget note: the three-scale ablation is single-epoch with memory_length=512; the scaling-law reference is fully-trained with memory_length=64. The two budgets differ and cannot be joined into a single scaling curve; the strict T2 verdict requires equal-compute multi-scale curves — see C16.8 caveat C1.)
...
- **The memory kernel is the dominant physical term**, with a relative contribution stable at 15–22% across all three scales, directly corresponding to the $\int\gamma$ colored-damping term the theory says Transformer discards. Raising the memory-kernel length from 64 to 512 (a v2.3 change) improved `cid_full`'s 10M perplexity from 23.62 (old, L=64) to 21.95 (L=512), about a 7% gain; both before/after results are retained (see C16.8).
- **The OU-over-FFT margin varies markedly with scale**: at 10M, FFT noise is catastrophic (perplexity ~157, the worst in the suite and the least stable in training), while at 100M FFT is only mildly worse (~12.67). This indicates the extreme 10M degradation is partly a small-scale training-instability effect; OU is supported as the physical default at all scales, but the "~7×" figure is a 10M-specific magnitude and must be reported per scale.
...
|---|---|---|---|
| **T1**: CID physical terms > Transformer | Yes | **Supported, and grows with scale** (3.35→3.71→4.09×; wins at ~1/10 params) | Multi-scale ablation (C16.1) + scaling law |
...
- **C7 (memory-kernel length is a mid-study engineering change)**: v2.3 raised memory_length from 64 to 512 (a post-hoc engineering adjustment). Both before (L=64, `cid_full` 10M perplexity 23.62) and after (L=512, 21.95) results are reported; nothing is silently replaced.
- **C8 (vortex's small ablation effect is not the relevant test)**: the vortex term's theoretical role is necessity-of-non-equilibrium (Prop. C3.3), to be tested by critical exponents rather than ablation loss; its small ablation effect should not be cited as "vortex is unimportant."
...
- **C10 (the OU-vs-FFT magnitude varies with scale)**: FFT's catastrophic 10M result (~157) narrows to mild at 100M (~12.67). The OU-over-FFT direction holds at all scales, but the "~7×" figure is a 10M-specific magnitude and must be reported per scale.
