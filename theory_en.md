...
**Falsifiable predictions and preliminary empirical results**: On this basis the paper proposes a falsifiable engineering target of **~5–10× parameter efficiency**, together with three critical-universality-class predictions independently confirmed in the biological brain: avalanche exponent τ ≈ 1.5 ([Beggs & Plenz, 2003](https://doi.org/10.1523/JNEUROSCI.23-35-11167.2003)), Hurst H ≈ 0.7 ([Linkenkaer-Hansen et al., 2001](https://doi.org/10.1523/JNEUROSCI.21-04-01370.2001)), and 1/f slope β ≈ 1 ([He, 2014](https://doi.org/10.1016/j.tics.2014.04.003)). **This version (v2.3) completes a multi-scale Phase 1 empirical study** (three scales 10M/30M/100M, Chinese MiniMind corpus, 3 seeds; a three-scale sweep of the 11-way ablation, critical exponents, and decode energy, plus an independent third-party reproduction). v2.3 makes two key updates over earlier versions: it **shows the borrowed §8.5 ET symmetric term to be intrinsically non-causal and therefore not realizable in a causal LM** (see the ET discussion below); and it **completes the 10M/30M/100M three-scale ablation sweep**.

**(i) The framework advantage (T1) is strongly supported and GROWS monotonically with scale.** Adding only UID's three **causal-safe** physical terms to an identical attention backbone, the `cid_full`/`transformer` perplexity ratio in the three-scale ablation (3 seeds, equal budget) **grows monotonically**: 73.58/23.62 = **3.12×** at 10M, 46.36/12.49 = **3.71×** at 30M, and 41.76/10.21 = **4.09×** at 100M. Most importantly, **`cid_full` at 10M (~4.8M non-embedding params, PPL 23.62) already substantially outperforms plain `transformer` at 100M (~49M params, PPL 41.76) — i.e. CID wins at roughly 1/10 the parameters**, a strong directional lower bound toward the 5–10× (T2) target. For reference, the fully-trained single point (≈200 tokens/param) at 10M gives `cid_full` 7.90 vs `transformer` 31.12 (3.94×), with CID's training loss (2.355) far below Transformer's (3.345), ruling out a "regularization-only" explanation. The colored-damping memory kernel is the single largest physical contributor (removal raises perplexity by 15–22% at every scale), and OU noise outperforms FFT spectral shaping (magnitude scale-dependent: ~7× at 10M, ~1.24× at 100M), consistent with Part I §14.2.

**(ii) "Attention is not all you need" is consistently replicated across all three scales and the fully-trained setting.** Known Transformer tricks give < 1% at each scale; `transformer_plus_tricks` (31.23) is even slightly worse than plain Transformer (31.12) while costing ~2.3× the compute. CID's advantage cannot be attributed to "more tricks."

**(iii) The critical-universality predictions receive partial support and, as anticipated, do not discriminate the baseline.** After the tooling fix (shuffle-surrogate Hurst = 0.519 ≈ 0.5; spectral-fit R² = 0.94): H = 0.803 is inside [0.6, 0.8] and far above the surrogate (**F4 PASS**); β = 0.572 is just below [0.7, 1.3] (**F3 not met**); the avalanche tail fails the power-law test (**F5 not met**); and η = 0.997, though > 0.5, is essentially identical to the baseline (0.998) and thus **non-discriminating** (**F6 inconclusive**). **None of the four exponents separate CID from a plain Transformer** — precisely as this abstract pre-states: these wide-band predictions have limited falsifying power and cannot separate CID from other self-organizing-critical models; they are descriptive corroboration only, **not** evidence that CID is superior to Transformer.

**(iv) Energy is cleanly measurable after the fix, but its decisive test awaits multiple scales.** Under a robust global idle baseline (61.9 W, spread 0.06 W; fixing a prior ~87 W per-family artifact), at iso-parameter decode `cid_full`'s per-token above-idle energy (0.160 mJ) is only ~**13%** higher than Transformer's (0.141 mJ) and *lower* than `transformer_plus_tricks` (0.164 mJ), with the fewest params and lowest peak power — i.e. "a ~3.9× quality gain for a ~13% per-token energy cost."

**All boundaries must be stated honestly**: the multi-scale advantage ratios (3.12–4.09×) are a **directional lower bound under the ablation budget (single-epoch training)**, and the 13% is an **iso-parameter overhead**; both are directionally consistent with the 5–10× (T2) and ≥3× (C13) targets but **do not directly test them**. T2 requires equal-compute multi-scale curves and C13 requires iso-performance curves, neither of which can hold at a single scale (the Transformer cannot reach CID's perplexity at that scale, so the iso-performance ratio is only a lower bound); both are deferred to Phase 1b (H100). Regarding the borrowed ET symmetric term ([Hoover et al., 2023](https://arxiv.org/abs/2302.07253); F8), v2.3 gives a **structural conclusion**: the symmetric second term of the ET energy's negative gradient requires key positions to depend on future query positions, causing future-token leakage in a causal LM (~0.11 measured), i.e. **ET's dual-softmax symmetry and autoregressive causality are mathematically incompatible**. The term is dropped, ET reduces to standard causal attention, and `cid_full ≡ cid_full_no_et`. This **supersedes** the earlier "F8 FAIL" wording: not "ET is useless" but **ET is inapplicable to causal LMs**; CID's entire advantage comes from its three **causal-safe** physical terms, independent of the borrowed ET (see Part I §16). In sum, **the genuinely discriminating decision points are parameter efficiency and correlation-length scaling**. UID's parameter-efficiency prediction is **complementary to, not in conflict with**, the Alman–Song–Gupta lower bounds.

(The three paragraphs "**On the honest weakening of the quantum-coherence-bonus wording**", "**Multi-Agent Intelligo-Dynamics**", and "**Complementarity with Logographic AI**" are unchanged.)

All references provide clickable DOIs or open-access links to primary sources, and every quantitative claim is labelled with an evidence grade (A verified / B theoretically rigorous but empirically pending / C falsifiable engineering target / D philosophical conjecture). The companion code repository ([github.com/gwailee/uid](https://github.com/gwailee/uid)) provides a reference CID implementation and a falsifiable-verification suite; all core predictions are reproducible within hours on a single GPU. **This version (v2.3) ships the full report of the multi-scale Phase 1 empirical study (per-seed records for all 11 variants across 10M/30M/100M × 3 seeds, commit hash, data SHA-256, negative results, and an independent third-party reproduction), following our commitment to "no selective reporting, no retroactive adjustment, negative results reported with equal prominence"; the FAILs of F3 (β) and F5 (avalanche), the non-discrimination of F6 (η), and F8 (the ET term being non-realizable due to non-causality) are all reported with equal prominence alongside the supported T1 trend, F4 (Hurst), and OU-over-FFT. The v2.3 engineering changes (memory kernel 64→512, ET dropped, critical-exponent tooling fixes) are all disclosed, with before/after results both retained and nothing silently replaced. We reiterate: the multi-scale ablation ratios are a directional lower bound under the ablation budget; the decisive tests of T2 and C13 must be performed on equal-compute multi-scale scaling curves and iso-performance energy curves, deferred to Phase 1b (H100). When citing this version's experimental results, attach the v2.3 commit hash and the per-item caveats listed in §6 of the report.**
...
All references in this paper provide clickable DOIs or open-access hyperlinks reaching the primary sources, and every quantitative claim is explicitly labelled with an evidence grade (A verified / B theoretically rigorous but empirically pending / C falsifiable engineering target / D philosophical conjecture). The companion code repository ([github.com/gwailee/uid](https://github.com/gwailee/uid)) provides a reference engineering implementation of CID and a falsifiable-verification suite; all core predictions are reproducible within hours on a single GPU. **This version (v2.3) ships the full report of the multi-scale Phase 1 study (per-seed records for all 11 variants across 10M/30M/100M × 3 seeds, the commit hash, data SHA-256, negative results, and an independent third-party reproduction), following "no selective reporting, no retroactive adjustment, negative results with equal prominence"; the FAILs of F3 (β) and F5 (avalanche), the non-discrimination of F6 (η), and F8 (the ET term being non-realizable due to non-causality) are reported with equal prominence alongside the supported T1 trend, F4 (Hurst), and OU-over-FFT. We reiterate: the multi-scale ablation ratios are a directional lower bound under the ablation budget; the decisive tests of T2 and C13 must be completed on equal-compute multi-scale scaling curves and iso-performance energy curves, deferred to Phase 1b (H100). When citing this version's results, attach the v2.3 commit hash and the per-item caveats in §6 of the report.**
...
### C16 Phase 1 Preliminary Empirical Results: Multi-Scale Ablation, Critical Exponents & Energy (v2.3)

This section reports the Phase 1 preliminary empirical results shipped with UID v2.3. Experiments were run on the Chinese MiniMind pretraining corpus (100k subset, ~10M tokens), with 3 random seeds ([42, 43, 44]), on a single NVIDIA RTX 4090, covering four experiment classes: **(1) a 10M/30M/100M three-scale sweep of the 11-way component ablation, (2) three-family scaling-law training (10M), (3) critical-exponent measurement, and (4) decode energy measurement**. The full report, per-seed raw records, commit hash, data SHA-256, and an independent third-party reproduction are in the companion repository [`results/phase1/REPORT.md`](https://github.com/gwailee/uid).

Two **boundary conditions** must be stated up front to preclude over-reading:

- The multi-scale ablation advantage ratios (3.12× at 10M, 3.71× at 30M, 4.09× at 100M) are **iso-parameter perplexity ratios under the ablation budget (single-epoch training)** and are a **directional lower bound** on parameter efficiency, **not** a strict measurement; the 5–10× parameter-efficiency promise (T2) requires multi-scale **equal-compute** scaling curves, deferred to full Phase 1 (1b, H100).
- The energy result is an **iso-parameter overhead**, **not** the C13 iso-performance (iso-PPL) energy-bonus verdict; the latter cannot hold at a single scale and is likewise deferred to Phase 1b.

We report all results under our commitment to "no selective reporting, no retroactive adjustment, negative results with equal prominence," including two FAILs, one non-discriminating INCONCLUSIVE, and a structural refutation of the ET term. The two v2.3 key updates over earlier versions (dropping the non-causal ET symmetric term; fixing the critical-exponent and energy toolchains) are all faithfully recorded, with before/after results retained and nothing silently replaced (see C16.8).

#### C16.1 Framework advantage (T1): grows monotonically with scale, CID wins at ~1/10 the parameters

With an attention backbone identical to the standard one, adding only UID's three **causal-safe** physical terms (vortex $v(\varphi)$, colored-damping memory kernel $\int\gamma$, OU colored noise $\xi$). The 10M/30M/100M three-scale ablation (1 epoch, 3 seeds, equal budget, memory_length=64):

| Scale | `cid_full` non-emb params | `cid_full` PPL | `transformer` PPL | Ratio |
|---|---|---|---|---|
...
1. **The advantage grows monotonically with scale** (3.12 → 3.71 → 4.09×). An architectural advantage that grows with size is the opposite of the classic failure mode in which a baseline catches up with more training or a larger scale.
2. **CID wins at ~1/10 the parameters**: `cid_full` at 10M (~4.8M non-embedding params, PPL 23.62) already substantially outperforms plain `transformer` at 100M (~49M params, PPL 41.76) — a **strong lower bound** on parameter efficiency, directionally consistent with the 5–10× (T2) target.
3. **Near-zero cross-seed variance**, indicating a stable architectural difference rather than a lucky seed.

As a **fully-trained reference**, three-family scaling-law training (≈200 tokens/param, 10M, 3 seeds) gives `transformer` 31.12, `transformer_plus_tricks` 31.23, and `cid_full` **7.90** (7.88/7.90/7.90, std ≈ 0.01) — a **3.94×** ratio at this fully-trained 10M point; CID's training loss (2.355) is also far below Transformer's (3.345), ruling out a "regularization-only" explanation.

(Budget note: both the three-scale ablation and the scaling-law reference use memory_length=64; their difference comes from the training budget (ablation 1 epoch vs scaling-law ~200 tokens/param), so they cannot be joined into a single scaling curve; the strict T2 verdict requires equal-compute curves — see C16.8 caveat C1.)

#### C16.2 "Attention is not all you need": consistently replicated across three scales and the fully-trained setting

Known Transformer tricks are useless in all settings:

- **Three-scale ablation**: the five Transformer variants (baseline, +conv, +linear, +noise, +all_tricks) differ by < 1% in perplexity at each of 10M/30M/100M.
- **Fully-trained scaling law**: `transformer_plus_tricks` (31.23) is even slightly worse than plain `transformer` (31.12) while costing ~**2.3×** the compute (wall-clock).
- **Energy** (see C16.5): `transformer_plus_tricks` consumes the highest per-token energy for the worst quality.
...
#### C16.3 Per-term contributions and the structural conclusion on the ET term

Per-term contributions (relative to `cid_full`, consistent across scales):

| Component toggled | Δ perplexity (10M / 30M / 100M) | Interpretation |
|---|---|---|
| Colored-damping memory kernel ($\int\gamma$) | +21.7% / +14.7% / +14.8% | **Largest single physical contributor**, stable across scale; maps to the colored-damping term Transformer discards |
| Vortex ($v(\varphi)$) | +0.2 to +0.4% | Small ablation effect; its *necessity* is tested by critical exponents (Prop. C3.3), see C16.4 |
| Colored noise (presence) | ≈ 0% | Presence matters little; the noise *type* (OU vs FFT), not presence, is what matters |
| OU → FFT noise | ~7× worse (10M) / ~1.24× (100M) | OU outperforms FFT, supporting §14.2; magnitude narrows with scale |

Two points must be stated honestly:

- **The memory kernel is the dominant physical term**, with a relative contribution stable at 15–22% across all three scales, directly corresponding to the $\int\gamma$ colored-damping term Transformer discards.
- **The OU-over-FFT margin varies markedly with scale**: at 10M FFT noise is catastrophic (perplexity ~157, the worst in the suite and least stable in training), while at 100M FFT is only mildly worse (~12.67). This indicates the extreme 10M degradation is partly a small-scale training-instability effect; OU is supported as the physical default at all scales, but the "~7×" figure is a 10M-specific magnitude and must be reported per scale.

**The v2.3 conclusion on the ET symmetric term (formerly pre-registered condition F8)**: earlier versions treated "enabling the ET symmetric term" as an ablation variant and reported no positive benefit (F8 FAIL). v2.3 provides a more fundamental **structural conclusion**: the negative gradient of the ET energy $E_{\text{ATT}}(g)=-\tfrac1\beta\sum_C\log\sum_{B\neq C}\exp(\beta K_B\!\cdot\!Q_C)$ has two terms, and its **symmetric second term** requires the update at key position $B$ to aggregate over query positions $C$ (with the only self-consistent direction being $C\ge B$), i.e. to **depend on future tokens**; in an autoregressive causal LM, a causality regression test measures ~0.11 future-token leakage from this term. **ET's dual-softmax symmetry and autoregressive causality are mathematically incompatible** — Hoover et al.'s (2023) ET and its Lyapunov monotonic-descent proof hold in the **non-causal** associative-memory setting, not in a causal LM. v2.3 therefore **drops the term** in the causal-LM reference implementation; `use_et_symmetric=True` reduces to standard causal attention, and `cid_full` is numerically identical to `cid_full_no_et` (bit-identical at all three scales).

This conclusion **supersedes** the earlier "F8 judged FAIL" wording. The correct reading is not "ET is useless" but **ET is inapplicable to causal language models**; accordingly, CID's entire advantage comes from its three **causal-safe** physical terms (vortex, colored-damping memory kernel, colored noise) and is independent of the borrowed, non-original ET component from Hoover et al. (2023). This structural result "purifies" the attribution and is fully consistent with the paper's standing statement that ET is not original to UID.

#### C16.4 Critical exponents (10M): partial support, no discrimination from the baseline

Critical exponents were measured on the trained `cid_full_10M` and `transformer_10M` checkpoints, with noise injection disabled, over 2000 sequences × 512 tokens. **v2.3 fixed three toolchain bugs** (see caveat C5); after the fixes, the estimators were validated by a **shuffle-surrogate Hurst = 0.519 ≈ 0.5** (the white-noise value) and a **spectral-fit $R^2 = 0.94$**. Corrected results:
...
- **F4 (Hurst) PASS**: H = 0.803 falls inside the predicted band and is **far above the shuffle surrogate (0.519)**, indicating genuine long-range positive correlation along the token axis, in the same direction as observations in human cortex ([Linkenkaer-Hansen et al., 2001](https://doi.org/10.1523/JNEUROSCI.21-04-01370.2001)).
...
- **F6 (η) INCONCLUSIVE**: η = 0.997 > 0.5, but is essentially identical to the Transformer baseline (0.998) — the metric $\eta=(\lambda_{\max}-\lambda_{\min})/(\lambda_{\max}+\lambda_{\min})$ saturates toward 1 for any heavy-tailed covariance spectrum and **has no discriminating power**, hence INCONCLUSIVE rather than PASS (a reformulation is recommended; see caveat C6).

**The most important honest conclusion of this section**: none of the four critical exponents distinguish CID from a plain Transformer (H 0.80 vs 0.81; β 0.57 vs 0.71; η 0.997 vs 0.998). This precisely confirms the stance already declared in the abstract — these wide-band universal-class predictions have limited falsifying power; they can rule out trivial cases (white noise) but cannot separate CID from other self-organizing-critical models. We therefore **do NOT use the critical exponents as discriminating evidence that CID is superior to Transformer**, only as descriptive corroboration that "CID exhibits brain-consistent critical statistics"; the genuinely discriminating decision points are the parameter-efficiency promise and correlation-length scaling (see C16.6).

#### C16.5 Decode energy (10M): iso-parameter overhead

Energy was measured in decode mode (64 new tokens per iteration, batch 8 × seq_len 256) with pynvml high-frequency sampling @ 25 Hz. **v2.3 used a robust global idle baseline** (3-window median = 61.9 W, inter-window spread only 0.06 W), fixing an earlier artifact in which per-family idle power differed by ~87 W (CID 124 W vs Transformer 211 W), which had made the above-idle column non-comparable (see caveat C5).
...
At iso-parameter, `cid_full`'s per-token above-idle energy is only ~**13%** higher than Transformer's, and *lower* than `transformer_plus_tricks` (which spends the most energy for the worst quality), while CID has the fewest parameters and the lowest peak power (259.9 W vs Transformer 275.1 W) — its overhead is dominated by serial latency rather than peak power, which is favorable for engineering optimization. Combined with the ~3.9× perplexity advantage: **CID buys a ~3.9× perplexity advantage for a ~13% per-token energy cost**.

We re-emphasize: this is an **iso-parameter overhead**, **not** the C13 "≥3× energy efficiency at equal performance" verdict (pre-registered condition F7). C13 must be measured at iso-performance (iso-PPL) — i.e. "how much energy a Transformer spends to reach CID's perplexity" — but a single-scale Transformer cannot reach that perplexity at all, so the iso-PPL energy ratio is currently undefined (the tool correctly refuses to extrapolate). F7 awaits the multi-scale curve and is deferred to Phase 1b.

#### C16.6 Phase 1 falsification scorecard

| Verdict | Pre-registered condition | Note |
|---|---|---|
| ✅ PASS (1) | F4 (Hurst) | H = 0.803 ∈ [0.6,0.8]; surrogate confirms genuine long-range correlation |
| ❌ FAIL (2) | F3 (β), F5 (avalanche) | β=0.572 slightly low; avalanche not power-law (with measurement-validity caveat) |
| ⚠️ INCONCLUSIVE (1) | F6 (η) | η>0.5 but indistinguishable from baseline; metric saturates |
| 🔧 RESOLVED_not_realizable (1) | F8 (ET symmetric term) | ET non-causal, not realizable in a causal LM, dropped; not "useless" but "inapplicable" |
| ⏳ ABSTAIN (3) | F1, F2 (equal-compute multi-scale curve), F7 (iso-PPL untestable at single scale) | The decision points for T2 and C13; deferred to Phase 1b (H100) |

It must be stressed: the FAILs of F3/F5 and the non-discrimination of F6 all concern the **B-grade (theoretically rigorous, empirically pending) critical-universality predictions**, which this paper has declared to have limited discriminating power and to be non-decisive; whereas F8's RESOLVED is a **structural** conclusion (ET incompatible with causality) concerning only the borrowed, non-original component from Hoover et al. (2023) and does not touch UID's own three physical terms. **The core decision points T2 (parameter efficiency) and C13 (energy) have not yet been decisively tested** — none of this section's positive results (T1 growing with scale, Hurst, OU>FFT) or negative results (β, avalanche, η) change that pending status; but the multi-scale ablation's "advantage grows with scale, CID wins at ~1/10 the parameters" already provides a strong directional lower bound for T2.

#### C16.7 Mapping to theoretical propositions

| Theoretical claim | Original to UID? | This round's verdict | Proper test |
|---|---|---|---|
| **T1**: CID physical terms > Transformer | Yes | **Supported, and grows with scale** (3.12→3.71→4.09×; wins at ~1/10 params) | Multi-scale ablation (C16.1) + scaling law |
| **T2**: 5–10× parameter efficiency | Yes | Strong directional support (ablation-budget lower bound ~10× params); verdict pending | Equal-compute multi-scale curve (F1/F2), Phase 1b |
...
| **§14.2**: OU > FFT colored noise | Yes | **Supported at all scales** (magnitude scale-dependent: 10M ~7×, 100M ~1.24×) | Ablation (C16.3) |
| **Colored-damping memory kernel ($\int\gamma$)** | Yes | **Supported, dominant** (+15–22% on removal at every scale) | Ablation (C16.3) |
| **C13**: ≥3× energy at iso-PPL (F7) | Yes | Untestable at single scale (iso-param overhead 1.13×) | Multi-scale iso-PPL (F7), Phase 1b |
| **ET symmetric term (§8.5)** | **No** (Hoover 2023) | **RESOLVED: non-causal, not realizable in causal LM, dropped** | Causality regression test (caveat C4) |

### C16.8 Limitations and per-item caveats

When citing any result in this section, the following per-item caveats must be cited together with it:

- **C1 (ablation budget vs equal-compute)**: the three-scale trend in C16.1 uses the ablation budget (single-epoch training), **not** equal-compute iso-FLOP. The strict T2 verdict (F1/F2) requires equal-compute multi-scale curves (Phase 1b, H100). "Wins at ~1/10 the parameters" is a lower-bound statement under the ablation budget.
...
- **C4 (the ET symmetric term is intrinsically non-causal)**: ET's symmetric second term requires key positions to depend on future query positions (a causality regression test measures ~0.11 leakage), mathematically incompatible with autoregressive causality. It is dropped in causal LMs (RESOLVED_not_realizable). This is not a refutation of Hoover et al.'s (2023) original non-causal ET but a setting-level conclusion that "ET is inapplicable to causal LMs."
- **C5 (critical-exponent and energy toolchains corrected in v2.3)**: three critical-exponent bugs were fixed — (a) the noise-OFF/ON contrast produced bit-identical outputs (a model in eval injects no stochastic noise, so the contrast is vacuous), replaced by a shuffle-surrogate control; (b) the per-sequence η covariance was ill-conditioned and saturated to ≈1, replaced by a global 50k-token covariance; (c) Hurst was biased down to ≈0.15 by an erroneous differencing-plus-correction, replaced by standard DFA-2 (validated by surrogate H = 0.519), and β had $R^2=0.14$ from per-sequence periodogram fitting, replaced by a Bartlett-averaged PSD ($R^2=0.94$). The energy measurement fixed the ~87 W per-family idle discrepancy (via a robust global idle baseline). **This section's F3/F4/F5/F6 verdicts and the energy numbers all use the corrected toolchain.** The avalanche detector's anomalously large `xmin` (~5857, leaving only ~1300 tail events) suggests its threshold/definition may be ill-suited at this scale, so F5's FAIL should carry this measurement-validity caveat.
- **C6 (the η metric lacks discriminating power)**: $\eta=(\lambda_{\max}-\lambda_{\min})/(\lambda_{\max}+\lambda_{\min})$ saturates for a heavy-tailed covariance spectrum, making CID (0.997) and Transformer (0.998) nearly identical. F6 is reported as INCONCLUSIVE rather than PASS; a participation-ratio or spectral-entropy reformulation is recommended before η is used as a discriminating test.
- **C7 (vortex's small ablation effect is not the relevant test)**: the vortex term's theoretical role is necessity-of-non-equilibrium (Prop. C3.3), to be tested by critical exponents rather than ablation loss; its small ablation effect should not be cited as "vortex is unimportant."
- **C8 (energy used random input tokens)**: the decode benchmark used random tokens (flagged in the results); per-token energy is largely insensitive to the input distribution, but a real-corpus re-measurement is recommended.
- **C9 (the OU-vs-FFT magnitude varies with scale)**: FFT's catastrophic 10M result (~157) narrows to mild at 100M (~12.67). The OU-over-FFT direction holds at all scales, but the "~7×" figure is a 10M-specific magnitude and must be reported per scale.

#### C16.9 Independent third-party reproduction

The core numbers in this section have been independently reproduced by Xingyu Zhao (Neutron Technology Application Research Center, Institute of Energy, Hefei Comprehensive National Science Center) in a fresh environment, on commit `53f2aa06` (10M scale, 3 seeds, RTX 4090), built from scratch following the official `requirements.txt` and documentation.

The reproduced numbers match our published values bit-for-bit: ablation Contrast A = 3.11× (z = 182.2); fully-trained scaling law (tokens/param = 200, 3 seeds) `cid_full` perplexity 7.89 ± 0.01, `transformer` 31.12 ± 0.16, ratio 3.94× ± 0.03; the order-of-magnitude separation of the two perplexity tiers (CID 22–29 / Transformer 73) is also reproduced. The reproduction **independently confirms our negative and neutral findings**: critical exponents are judged `emergence_not_confirmed` and cannot distinguish CID from Transformer (H 0.77 vs 0.81, β 0.51 vs 0.64, η 0.998 vs 0.997); iso-parameter decode energy is ~20% higher for CID (same direction as our ~13% prefill figure); raw inference throughput is ~27% higher for Transformer; and iso-PPL energy is undecidable at this scale because the perplexity ranges do not overlap. The report also independently flagged several engineering issues (an unsatisfiable `typeguard` pin in `requirements.txt`, the `transformers<5.0` upper bound, a missing sampling cap in the critical-exponent statistics, and inconsistent training-budget defaults across entry points), which we have used to revise the repository. The full reproduction report and raw data are in the companion repository [`results/phase1/independent-reports/REPRODUCTION_en.md`](https://github.com/gwailee/uid).

> **Note**: the reproduction used commit `53f2aa06` (before the v2.3 ET fix), whose critical-exponent tool is the pre-fix version, so the reproduced Hurst (0.77) differs slightly from our corrected value (0.803), though in the same direction (both inside [0.6,0.8] and both non-discriminating vs the baseline).

We note honestly that this reproduction further corroborates the **robustness of our parameter-efficiency direction (3.12× single-epoch → 3.94× fully-trained, growing with training)** while also independently reproducing our **as-yet-unestablished decisive predictions** — the iso-PPL energy bonus (C13) and critical emergence are both undecidable/unsupported at the single 10M point. This is fully consistent with the boundaries listed in C16.8.
