...
本仓库 Phase 1 核心结果已由赵星宇（合肥综合性国家科学中心能源研究院中子技术应用研究中心）在独立环境下复现（commit `53f2aa06`）。关键数值逐位吻合：消融对照 A **3.11× / z=182.2**；充分训练缩放律 **cid 7.89 / tf 31.12（3.94× ± 0.03）**。复现同等地验证了负面/中性结论——临界指数无法区分 CID 与 Transformer、等参数能耗 CID 高约 20%、推理吞吐 Transformer 快约 27%、iso-PPL 能效在该规模不可判定——并独立发现若干工程问题（已据此修订）。完整报告见 [`results/phase1/independent-reports/REPRODUCTION_zh.md`](./results/phase1/independent-reports/REPRODUCTION_zh.md)。
---
